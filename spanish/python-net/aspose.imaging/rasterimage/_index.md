...
type: docs
weight: 7060
url: /es/python-net/aspose.imaging/rasterimage/
---
...
| [premultiply_components](#premultiply_components2) | bool | r/w | Obtiene o establece un valor que indica si los componentes de la imagen deben ser premultiplicados. |
...
| [raw_data_format](#raw_data_format3) | [PixelDataFormat](/imaging/python-net/aspose.imaging/pixeldataformat/) | r | Obtiene el formato de datos sin procesar. |
...
| auto_brightness_contrast() | Normalización automática adaptativa de brillo y contraste para toda la imagen. |
...
| [crop(rectangle)](#crop_rectangle_31) | Recorta el rectángulo especificado. |
...
| [resize(new_width, new_height, settings)](#resize_new_width_new_height_settings_82) | Redimensiona la imagen con opciones extendidas. |
...
| [save_pixels(rectangle, pixels)](#save_pixels_rectangle_pixels_106) | Guarda los píxeles. |
...
| [try_set_metadata(metadata)](#try_set_metadata_metadata_117) | Intenta establecer una instancia de _metadata_, si esta instancia de [Image](/imaging/python-net/aspose.imaging/image/) admite e implementa el tipo [IImageMetadataFormat](/imaging/python-net/aspose.imaging.metadata/iimagemetadataformat/). |
...
Obtiene o establece un valor que indica si los componentes de la imagen deben ser premultiplicados.
...
with Image.load(path_join(directory, "sample.png")) as image:
	# Cachea todos los datos de píxeles para que no se realice una carga adicional de datos desde el flujo de datos subyacente
	image.cache_daata()
...
with Image.load(join_path(directory, "sample.png")) as image:
	rasterImage = as_of(image, RasterImage)
	# Realizar tramado por umbral usando una paleta de colores de 4 bits que contiene 16 colores.
	# Cuantos más bits se especifiquen, mayor será la calidad y el mayor tamaño de la imagen de salida.
...
with Image.load(join_path(directory, "sample.png")) as image:
	...
	# Realizar tramado Floyd usando una paleta de colores de 1 bit que contiene solo 2 colores: negro y blanco.
	# Cuantos más bits se especifiquen, mayor será la calidad y el mayor tamaño de la imagen de salida.
	# Tenga en cuenta que solo se admiten paletas de 1 bit, 4 bits y 8 bits en este momento.
...
with Image.load("sample.png") as image:
	...
	# Obtener una representación entera del color del píxel superior izquierdo de la imagen.
	color = rasterImage.get_argb_32_pixel(0, 0)

	# Para obtener los valores de los componentes de color individuales, desplaza el valor del color un número correspondiente de bits
	alpha = (color >> 24) & 0xff
	red = (color >> 16) & 0xff
	green = (color >> 8) & 0xff
	blue = (color >> 0) & 0xff
...
with Image.load("sample.png") as image:
	rasterImage = as_of(image, RasterImage)
	# Obtener el color del píxel superior izquierdo de la imagen.
...
with Image.load("alpha.png") as image:
	...
	# Cargar píxeles para toda la imagen. Cualquier parte rectangular de la imagen puede especificarse como un parámetro del método aspose.imaging.RasterImage.load_argb_32_pixels(rectangle).
...
with Image.load("16rgba.png") as image:
	rasterImage = as_of(image, RasterImage)
	# Cargar píxeles para toda la imagen. Cualquier parte rectangular de la imagen puede especificarse como un parámetro del método aspose.imaging.RasterImage.load_argb_64_pixels.
	# Tenga en cuenta que la propia imagen debe tener 16 bits por muestra, porque aspose.imaging.RasterImage.load_argb_64_pixels no funciona con 8 bits por muestra.
	# Para trabajar con 8 bits por muestra, por favor use el buen y viejo método aspose.imaging.RasterImage.load_argb_64_pixels.
...
with Image.load("alpha.png") as image:
	rasterImage = as_of(image, RasterImage)
	# Cargar píxeles para toda la imagen. Cualquier parte rectangular de la imagen puede especificarse como un parámetro del método aspose.imaging.RasterImage.load_pixels.
...
# Elimine la exploración sesgada con los parámetros predeterminados
