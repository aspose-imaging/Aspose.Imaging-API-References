---
title: EmfPlusComment
second_title: Aspose.Imaging for .NET API Referansı
description: EmfPlusComment kaydı isteğe bağlı özel verileri belirtir.
type: docs
weight: 5880
url: /tr/net/aspose.imaging.fileformats.emf.emfplus.records/emfpluscomment/
---
## EmfPlusComment class

EmfPlusComment kaydı, isteğe bağlı özel verileri belirtir.

```csharp
public sealed class EmfPlusComment : EmfPlusRecord
```

## yapıcılar

| İsim | Tanım |
| --- | --- |
| [EmfPlusComment](emfpluscomment)(EmfPlusRecord) | Yeni bir örneğini başlatır[`EmfPlusComment`](../emfpluscomment) sınıf. |

## Özellikleri

| İsim | Tanım |
| --- | --- |
| virtual [DataSize](../../aspose.imaging.fileformats.emf.emfplus.records/emfplusrecord/datasize) { get; set; } | Takip eden RecordData alanındaki 32-bit hizalı bayt veri sayısını tanımlaması ZORUNLU olan 32-bit işaretsiz bir tamsayı alır veya ayarlar. Bu sayı, 12 baytlık kayıt başlığını içermez. |
| override [Flags](../../aspose.imaging.fileformats.emf.emfplus.records/emfpluscomment/flags) { get; set; } | Kullanılmayan 16 bitlik işaretsiz bir tamsayı alır veya ayarlar. Bu alan, sıfır olarak AYARLANMALIDIR ve alındı alındığında yok sayılmalıdır MUTLAKA |
| [PrivateData](../../aspose.imaging.fileformats.emf.emfplus.records/emfpluscomment/privatedata) { get; set; } | Özel verilerin DataSize uzunluğunda bayt dizisini alır veya ayarlar. Bunu izleyen kayda özel veri baytları. |
| virtual [Size](../../aspose.imaging.fileformats.emf.emfplus.records/emfplusrecord/size) { get; set; } | 12 baytlık kayıt başlığı ve kayda özel veriler dahil olmak üzere tüm kayıttaki 32 bit hizalanmış bayt sayısını belirten 32 bit işaretsiz bir tamsayı alır veya ayarlar. |
| [Type](../../aspose.imaging.fileformats.emf.emfplus.records/emfplusrecord/type) { get; } | Kayıt türünü tanımlayan 16 bitlik işaretsiz bir tamsayı alır. |

### Ayrıca bakınız

* class [EmfPlusRecord](../emfplusrecord)
* ad alanı [Aspose.Imaging.FileFormats.Emf.EmfPlus.Records](../../aspose.imaging.fileformats.emf.emfplus.records)
* toplantı [Aspose.Imaging](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Imaging.dll -->
