---
title: "StreamContainer 类"
type: docs
weight: 7340
url: /zh/python-net/aspose.imaging/streamcontainer/
---
...
| **Name** | **描述** |
| :- | :- |
| [StreamContainer(stream)](#StreamContainer_stream_1) | 初始化一个新的 [StreamContainer](/imaging/python-net/aspose.imaging/streamcontainer/) 类实例。 |
| [StreamContainer(stream, dispose_stream)](#StreamContainer_stream_dispose_stream_2) | 初始化一个新的 [StreamContainer](/imaging/python-net/aspose.imaging/streamcontainer/) 类实例。 |
## **Properties**
| **Name** | **Type** | **Access** | **描述** |
| :- | :- | :- | :- |
...
| [read(buffer, offset, count)](#read_buffer_offset_count_1) | 从当前流读取一系列字节，并将流中的位置前移读取的字节数。 |
| [read(bytes)](#read_bytes_2) | 读取字节以填充指定的字节缓冲区。 |
| [read_byte()](#read_byte__3) | 从流中读取一个字节，并将流中的位置前移一个字节；如果已到达流的末尾，则返回 -1。 |
| [save(destination_stream)](#save_destination_stream_4) | 将流的数据保存（复制）到指定的流。使用默认缓冲区大小 [StreamContainer.READ_WRITE_BYTES_COUNT](/imaging/python-net/aspose.imaging/streamcontainer/) 和流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save(destination_stream, buffer_size)](#save_destination_stream_buffer_size_5) | 将所有流的数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save(destination_stream, buffer_size, length)](#save_destination_stream_buffer_size_length_6) | 将流的数据保存（复制）到指定的流。 |
| [save(file_path)](#save_file_path_7) | 将流的数据保存（复制）到指定的流。使用默认缓冲区大小 [StreamContainer.READ_WRITE_BYTES_COUNT](/imaging/python-net/aspose.imaging/streamcontainer/) 和流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save(file_path, buffer_size)](#save_file_path_buffer_size_8) | 将流的数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save(file_path, buffer_size, length)](#save_file_path_buffer_size_length_9) | 将流的数据保存（复制）到指定的流。 |
| [save_to_stream(destination_stream)](#save_to_stream_destination_stream_10) | 将流的数据保存（复制）到指定的流。使用默认缓冲区大小 [StreamContainer.READ_WRITE_BYTES_COUNT](/imaging/python-net/aspose.imaging/streamcontainer/) 和流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save_to_stream_with_buf_size(destination_stream, buffer_size)](#save_to_stream_with_buf_size_destination_stream_buffer_size_11) | 将所有流的数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save_to_stream_with_buf_size_and_len(destination_stream, buffer_size, length)](#save_to_stream_with_buf_size_and_len_destination_stream_buffer_size_length_12) | 将流的数据保存（复制）到指定的流。 |
| [save_with_buf_size(file_path, buffer_size)](#save_with_buf_size_file_path_buffer_size_13) | 将流的数据保存（复制）到指定的流。使用流的 [StreamContainer.length](/imaging/python-net/aspose.imaging/streamcontainer/) 值。 |
| [save_with_buf_size_and_len(file_path, buffer_size, length)](#save_with_buf_size_and_len_file_path_buffer_size_length_14) | 将流的数据保存（复制）到指定的流。 |
| [seek(offset, origin)](#seek_offset_origin_15) | 设置当前流中的位置。 |
| seek_begin() | 将流位置设置为流的起始位置。此值表示相对于在 StreamContainer 构造函数中传入的起始流位置的偏移量。 |
| [to_bytes()](#to_bytes__16) | 将流数据转换为 int 数组。 |
| [to_bytes(position, bytes_count)](#to_bytes_position_bytes_count_17) | 将流数据转换为 int 数组。 |
| [write(buffer, offset, count)](#write_buffer_offset_count_18) | 将一系列字节写入当前流，并将此流中的当前位置向前移动已写入的字节数。 |
| [write(bytes)](#write_bytes_19) | 将所有指定的字节写入流。 |
| [write_byte(value)](#write_byte_value_20) | 在流的当前位写入一个字节，并将流中的位置前移一个字节。 |
| [write_to(stream_container)](#write_to_stream_container_21) | 将包含的数据复制到另一个 [StreamContainer](/imaging/python-net/aspose.imaging/streamcontainer/)。 |
| [write_to(stream_container, length)](#write_to_stream_container_length_22) | 将包含的数据复制到另一个 [StreamContainer](/imaging/python-net/aspose.imaging/streamcontainer/)。 |
...
初始化一个新的 [StreamContainer](/imaging/python-net/aspose.imaging/streamcontainer/) 类实例。
