...
type: docs
weight: 40
url: /zh/python-net/aspose.imaging.fileformats.svg/svgimage/
---
...
| **Name** | **描述** |
| :- | :- |
| [SvgImage(path)](#SvgImage_path_1) | 实例化 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新对象，利用 <br/> 指定的路径来定位和加载图像。此构造函数促进 <br/> 从外部文件创建 SVG 图像实例，实现软件系统和工作流的无缝集成。 |
| [SvgImage(stream)](#SvgImage_stream_2) | 创建 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例，加载图像 <br/> 来自提供的流。此构造函数使得直接从流中加载 SVG <br/> 图像成为可能，提升在软件应用中处理图像 <br/> 资源的灵活性和效率。 |
| [SvgImage(svg_options, width, height)](#SvgImage_svg_options_width_height_3) | 创建 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例，使用指定的 SVG <br/> 选项、图像宽度和高度参数。此构造函数使 <br/> 开发者能够使用自定义选项和尺寸初始化 SVG 图像，<br/> 提供在管理 SVG 内容和布局方面的灵活性。 |
| [SvgImage(width, height)](#SvgImage_width_height_4) | 实例化具有指定宽度和 <br/> 高度的 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 对象。此构造函数允许开发者创建具有预定义 <br/> 尺寸的 SVG 图像，便于在初始化期间精确控制图像大小。 |
## **Properties**
| **Name** | **Type** | **Access** | **描述** |
| :- | :- | :- | :- |
...
| bits_per_pixel | int | r | 检索图像的每像素位数。需要注意的是 <br/> 此参数不适用于矢量图像，因为它们不以像素计量。此属性提供有关图像颜色 <br/> 深度的关键信息，有助于处理和操作任务。 |
...
| file_format | [FileFormat](/imaging/python-net/aspose.imaging/fileformat/) | r | 检索图像的文件格式，提供用于 <br/> 处理和兼容性检查的关键元数据。此属性在 <br/> 确定处理图像数据的适当解码和编码策略方面起着重要作用，能够在不同系统和应用之间有效地处理图像数据。 |
...
| is_cached | bool | r | 检索一个布尔值，指示对象的数据当前是否已 <br/> 缓存，从而消除额外读取数据的需求。此 <br/> 属性提供当前缓存状态的洞察，优化数据 <br/> 检索和处理工作流，以提升性能和效率。 |
...
| **Name** | **描述** |
| :- | :- |
| cache_data() | 缓存数据并确保不再从底层 [DataStreamSupporter.data_stream_container](/imaging/python-net/aspose.imaging/datastreamsupporter/) 加载数据。此 <br/> 优化通过消除冗余的数据检索 <br/> 操作来提升性能，尤其在需要频繁访问 <br/> 图像数据的场景中受益匪浅。 |
...
| [create_from_stream(stream)](#create_from_stream_stream_20) | 初始化 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例。 |
...
| [get_original_options()](#get_original_options__30) | 根据原始文件设置获取选项。<br/>            这有助于保持原始图像的位深度和其他参数不变。<br/>            例如，如果我们加载一个每像素 1 位的黑白 PNG 图像，然后使用<br/>            [DataStreamSupporter.save(file_path)](/imaging/python-net/aspose.imaging/datastreamsupporter/) 方法保存，它将生成每像素 8 位的输出 PNG 图像。<br/>            为避免这种情况并以每像素 1 位保存 PNG 图像，请使用此方法获取相应的保存选项，并将其<br/>            作为第二个参数传递给 [Image.save(file_path, options)](/imaging/python-net/aspose.imaging/image/) 方法。 |
| [get_proportional_height(width, height, new_width)](#get_proportional_height_width_height_new_width_31) | 获取比例高度。 |
| [get_proportional_width(width, height, new_height)](#get_proportional_width_width_height_new_height_32) | 获取比例宽度。 |
| [get_serialized_stream(image_options, clipping_rectangle, page_number)](#get_serialized_stream_image_options_clipping_rectangle_page_number_33) | 转换为 aps。 |
| [load(file_path)](#load_file_path_34) | 从指定的文件路径或 URL 加载新图像。<br/>            如果 _filePath_ 是文件路径，方法仅打开该文件。<br/>            如果 _filePath_ 是 URL，方法会下载文件，将其存为临时文件，然后打开它。 |
| [load(file_path, load_options)](#load_file_path_load_options_35) | 从指定的文件路径或 URL 加载新图像。<br/>            如果 _filePath_ 是文件路径，方法仅打开该文件。<br/>            如果 _filePath_ 是 URL，方法会下载文件，将其存为临时文件，然后打开它。 |
| [load(stream)](#load_stream_36) | 加载 SVG 部分。 |
| [load(stream, load_options)](#load_stream_load_options_37) | 从指定的流加载新图像。 |
| [load_stream(stream)](#load_stream_stream_38) | 从指定的流加载新图像。 |
| [load_stream_with_options(stream, load_options)](#load_stream_with_options_stream_load_options_39) | 从指定的流加载新图像。 |
| [load_with_options(file_path, load_options)](#load_with_options_file_path_load_options_40) | 从指定的文件路径或 URL 加载新图像。<br/>            如果 _filePath_ 是文件路径，方法仅打开该文件。<br/>            如果 _filePath_ 是 URL，方法会下载文件，将其存为临时文件，然后打开它。 |
| remove_background() | 移除背景。 |
| [remove_background(settings)](#remove_background_settings_41) | 移除背景。 |
| remove_metadata() | 移除元数据。 |
| [resize(new_width, new_height)](#resize_new_width_new_height_42) | 调整图像大小。使用默认的 [ResizeType.NEAREST_NEIGHBOUR_RESAMPLE](/imaging/python-net/aspose.imaging/resizetype/)。 |
| [resize(new_width, new_height, resize_type)](#resize_new_width_new_height_resize_type_43) | 调整图像大小以适应指定的尺寸，同时保持其宽高 <br/>            比例。此方法提供了一种方便的方式来调整图像的大小 <br/>            而不失真其比例，确保根据所需尺寸实现最佳显示或存储 <br/>            。 |
| [resize(new_width, new_height, settings)](#resize_new_width_new_height_settings_44) | 使用扩展选项调整图像大小。 |
| [resize_by_settings(new_width, new_height, settings)](#resize_by_settings_new_width_new_height_settings_45) | 调整图像大小。 |
| [resize_by_type(new_width, new_height, resize_type)](#resize_by_type_new_width_new_height_resize_type_46) | 调整图像大小。 |
| [resize_height_proportionally(new_height)](#resize_height_proportionally_new_height_47) | 按比例调整高度。默认使用 [ResizeType.NEAREST_NEIGHBOUR_RESAMPLE](/imaging/python-net/aspose.imaging/resizetype/)。 |
| [resize_height_proportionally(new_height, resize_type)](#resize_height_proportionally_new_height_resize_type_48) | 按比例调整高度。 |
| [resize_height_proportionally(new_height, settings)](#resize_height_proportionally_new_height_settings_49) | 按比例调整高度。 |
| [resize_height_proportionally_settings(new_height, settings)](#resize_height_proportionally_settings_new_height_settings_50) | 按比例调整高度。 |
| [resize_width_proportionally(new_width)](#resize_width_proportionally_new_width_51) | 按比例调整宽度。默认使用 [ResizeType.NEAREST_NEIGHBOUR_RESAMPLE](/imaging/python-net/aspose.imaging/resizetype/)。 |
| [resize_width_proportionally(new_width, resize_type)](#resize_width_proportionally_new_width_resize_type_52) | 按比例调整宽度。 |
| [resize_width_proportionally(new_width, settings)](#resize_width_proportionally_new_width_settings_53) | 按比例调整宽度。 |
| [resize_width_proportionally_settings(new_width, settings)](#resize_width_proportionally_settings_new_width_settings_54) | 按比例调整宽度。 |
| [rotate(angle)](#rotate_angle_55) | 围绕中心旋转图像。 |
| [rotate_flip(rotate_flip_type)](#rotate_flip_rotate_flip_type_56) | 旋转、翻转或同时旋转和翻转图像。 |
| save() | 将图像数据保存到底层流中。 |
| [save(file_path)](#save_file_path_57) | 将图像保存到指定的文件位置。 |
| [save(file_path, options)](#save_file_path_options_58) | 根据保存选项，将对象的数据保存到指定文件位置的指定文件格式中。 |
| [save(file_path, options, bounds_rectangle)](#save_file_path_options_bounds_rectangle_59) | 根据保存选项，将对象的数据保存到指定文件位置的指定文件格式中。 |
| [save(file_path, over_write)](#save_file_path_over_write_60) | 将对象的数据保存到指定的文件位置。 |
| [save(stream)](#save_stream_61) | 保存数据。 |
| [save(stream, options_base)](#save_stream_options_base_62) | 根据保存选项，将图像的数据保存到指定流中的指定文件格式。 |
| [save(stream, options_base, bounds_rectangle)](#save_stream_options_base_bounds_rectangle_63) | 根据保存选项，将图像的数据保存到指定流中的指定文件格式。 |
| [save_to_stream(stream)](#save_to_stream_stream_64) | 将对象的数据保存到指定的流中。 |
| [save_to_stream_with_options(stream, options_base)](#save_to_stream_with_options_stream_options_base_65) | 根据保存选项，将图像的数据保存到指定流中的指定文件格式。 |
| [save_to_stream_with_options_rect(stream, options_base, bounds_rectangle)](#save_to_stream_with_options_rect_stream_options_base_bounds_rectangle_66) | 根据保存选项，将图像的数据保存到指定流中的指定文件格式。 |
| [save_with_options(file_path, options)](#save_with_options_file_path_options_67) | 根据保存选项，将对象的数据保存到指定文件位置的指定文件格式中。 |
| [save_with_options_rect(file_path, options, bounds_rectangle)](#save_with_options_rect_file_path_options_bounds_rectangle_68) | 根据保存选项，将对象的数据保存到指定文件位置的指定文件格式中。 |
| [set_palette(palette, update_colors)](#set_palette_palette_update_colors_69) | 将指定的调色板应用于图像，启用对颜色 <br/>            方案的自定义，以满足美学或功能需求。此方法在管理颜色调色板方面提供灵活性 <br/>            以适应各种设计或应用需求。 |
| [try_set_metadata(metadata)](#try_set_metadata_metadata_70) | 尝试设置一个 _metadata_ 实例，前提是此 [Image](/imaging/python-net/aspose.imaging/image/) 实例支持并实现 [IImageMetadataFormat](/imaging/python-net/aspose.imaging.metadata/iimagemetadataformat/) 实例。 |
...
实例化 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新对象，利用 <br/> 指定的路径来定位和加载图像。此构造函数促进 <br/> 从外部文件创建 SVG 图像实例，实现软件系统和工作流的无缝集成。
...
创建 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例，加载图像 <br/> 来自提供的流。此构造函数使得直接从流中加载 SVG <br/> 图像成为可能，提升在软件应用中处理图像 <br/> 资源的灵活性和效率。
...
创建 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例，使用指定的 SVG <br/> 选项、图像宽度和高度参数。此构造函数使 <br/> 开发者能够使用自定义选项和尺寸初始化 SVG 图像，<br/> 提供在管理 SVG 内容和布局方面的灵活性。

**Parameters:**

| 参数 | Type | Description |
| :- | :- | :- |
| svg_options | [SvgOptions](/imaging/python-net/aspose.imaging.imageoptions/svgoptions/) | SVG 选项。 |
| width | int | 图像宽度。 |
| height | int | 图像高度。 |
...
实例化具有指定宽度和 <br/> 高度的 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 对象。此构造函数允许开发者创建具有预定义 <br/> 尺寸的 SVG 图像，便于在初始化期间精确控制图像大小。
...
初始化 [SvgImage](/imaging/python-net/aspose.imaging.fileformats.svg/svgimage/) 类的新实例。
...
调整图像大小以适应指定的尺寸，同时保持其宽高 <br/>            比例。此方法提供了一种方便的方式来调整图像的大小 <br/>            而不失真其比例，确保根据所需尺寸实现最佳显示或存储 <br/>            。
...
将指定的调色板应用于图像，启用对颜色 <br/>            方案的自定义，以满足美学或功能需求。此方法在管理颜色调色板方面提供灵活性 <br/>            以适应各种设计或应用需求。
