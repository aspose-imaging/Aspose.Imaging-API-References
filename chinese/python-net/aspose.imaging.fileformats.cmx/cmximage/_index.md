...
type: docs
weight: 90
url: /zh/python-net/aspose.imaging.fileformats.cmx/cmximage/
---
...
| **Name** | **描述** |
| :- | :- |
| [CmxImage(stream_container, load_options)](#CmxImage_stream_container_load_options_1) | 通过初始化<br/>            使用 streamContainer 和 loadOptions 参数的新实例，开始无缝使用 [CmxImage](/imaging/python-net/aspose.imaging.fileformats.cmx/cmximage/) 类。适用于开发者<br/>            寻求以便捷方式从各种数据源加载 CMX 图像并根据需要自定义加载过程。 |
## **Properties**
| **Name** | **Type** | **Access** | **描述** |
| :- | :- | :- | :- |
...
| bits_per_pixel | int | r | 使用此用户友好的属性轻松获取图像的位深度。<br/>            适用于希望确定图像细节级别或颜色深度的开发者，确保准确的处理和操作。 |
...
| cmx_page | [CmxPage](/imaging/python-net/aspose.imaging.fileformats.cmx.objectmodel/cmxpage/) | r | 使用此直观属性轻松检索图像的 CMX 页面。<br/>            适用于希望快速访问 CMX 图像中各个页面的开发者，确保高效的导航和管理。 |
...
| default_page | [Image](/imaging/python-net/aspose.imaging/image/) | r | 使用此直观属性轻松检索图像的默认页面。<br/>            适用于希望快速访问图像主页面的开发者，确保高效的导航和管理。 |
| disposed | bool | r | 获取一个值，指示此实例是否已释放。 |
| document | [CmxDocument](/imaging/python-net/aspose.imaging.fileformats.cmx.objectmodel/cmxdocument/) | r | 使用此直观属性轻松检索 CMX 文档。适用于<br/>            希望访问或修改 CMX 图像的开发者，确保其应用的灵活性和效率。 |
| exif_data | [ExifData](/imaging/python-net/aspose.imaging.exif/exifdata/) | r/w | 获取或设置 Exif 实例。 |
| file_format | [FileFormat](/imaging/python-net/aspose.imaging/fileformat/) | r | 使用此用户友好的属性轻松获取图像的文件格式。<br/>            适用于希望动态确定图像格式的开发者，确保其应用的兼容性和准确处理。 |
| has_background_color | bool | r/w | 获取或设置指示图像是否具有背景颜色的值。 |
| height | int | r | 获取图像高度。 |
| height_f | float | r | 使用此<br/>            用户友好的属性轻松获取对象的高度（以英寸为单位）。适用于希望获取精确尺寸信息以实现有效布局和展示的开发者。 |
| interrupt_monitor | [InterruptMonitor](/imaging/python-net/aspose.imaging.multithreading/interruptmonitor/) | r/w | 获取或设置中断监视器。 |
| is_cached | bool | r | 确定对象的数据是否已缓存，从而消除读取数据的需求。适用于希望通过高效利用缓存数据来优化性能的开发者，确保更快访问对象信息。 |
| metadata | [ImageMetadata](/imaging/python-net/aspose.imaging.metadata/imagemetadata/) | r | 获取图像元数据。 |
| page_count | int | r | 使用此直观属性检索图像的总页数。<br/>            适用于希望动态管理多页图像的开发者，确保高效的导航和内容操作。 |
| [pages](#pages1) | [Image[]](/imaging/python-net/aspose.imaging/image/) | r | 使用此直观属性无缝检索图像的页面。<br/>            适用于希望访问和操作多页图像中各个页面的开发者，确保高效的导航和处理。 |
...
| width | int | r | 获取图像宽度。 |
| width_f | float | r | 使用此直观属性获取对象的宽度（英寸）。<br/>            适用于希望在应用中获得对象精确测量的开发者，确保准确的布局和展示。 |
| xmp_data | [XmpPacketWrapper](/imaging/python-net/aspose.imaging.xmp/xmppacketwrapper/) | r/w | 获取或设置 Xmp 数据。 |
## **Methods**
| **Name** | **描述** |
| :- | :- |
| cache_data() | 使用此便捷方法缓存数据，以防止从底层来源<br/>            [DataStreamSupporter.data_stream_container](/imaging/python-net/aspose.imaging/datastreamsupporter/)<br/>            进行额外加载。适用于希望通过预加载数据来优化性能的开发者，确保其应用更快访问和更流畅的运行。 |
...
| [set_palette(palette, update_colors)](#set_palette_palette_update_colors_68) | 使用此直观方法自定义图像的颜色调色板。<br/>            适用于希望动态应用特定配色方案或调整的开发者，确保对图像视觉外观的精确控制。 |
...
通过初始化<br/>            使用 streamContainer 和 loadOptions 参数的新实例，开始无缝使用 [CmxImage](/imaging/python-net/aspose.imaging.fileformats.cmx/cmximage/) 类。适用于开发者<br/>            寻求以便捷方式从各种数据源加载 CMX 图像并根据需要自定义加载过程。
...
使用此直观属性无缝检索图像的页面。<br/>            适用于希望访问和操作多页图像中各个页面的开发者，确保高效的导航和处理。
...
使用此直观方法自定义图像的颜色调色板。<br/>            适用于希望动态应用特定配色方案或调整的开发者，确保对图像视觉外观的精确控制。
