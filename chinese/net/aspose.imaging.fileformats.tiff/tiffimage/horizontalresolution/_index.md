...
description: "TiffImage 属性。检索指定 Image 的水平分辨率（以每英寸像素为单位），以便进行精确的调整和渲染。轻松访问关键图像元数据，提升图像处理工作流的流畅性，增强用户体验。"
type: docs
weight: 70
...
检索指定[`Image`](../../../aspose.imaging/image/)的水平分辨率（以每英寸像素为单位），以实现精确的调整和渲染功能。轻松访问关键图像元数据，提升图像处理工作流的流畅性，增强用户体验。
