...
description: ModifyWorldTransformMode 枚举定义了使用指定变换数据 修改播放中当前定义的世界空间到页面空间变换的模式设备上下文
type: docs
weight: 2770
...
ModifyWorldTransformMode 枚举定义了使用指定变换数据 修改播放中当前定义的世界空间到页面空间变换的模式设备上下文。
...
| MWT_LEFTMULTIPLY | `2` | 乘以当前变换。在这种模式下，指定的变换数据是左被乘数，并且 当前在播放设备上下文中定义的变换是右被乘数 |
| MWT_RIGHTMULTIPLY | `3` | 乘以当前变换。在这种模式下，指定的变换数据是右被乘数 并且当前在播放设备上下文中定义的变换是左被乘数 |
