...
description: EmrComment 枚举定义公共评论记录可以 包含的数据类型如第 2.3.3.4 节所述
type: docs
weight: 2620
...
EmrComment 枚举定义公共评论记录可以 包含的数据类型，如第 2.3.3.4 节所述。
...
| EMR_COMMENT_BEGINGROUP | `2` | 此注释记录标识一组绘图记录的开始。它标识 EMF 元文件 |
...
| EMR_COMMENT_UNICODE_STRING | `64` | 此注释记录是保留的，不得在 EMF 元文件中使用 |
| EMR_COMMENT_UNICODE_END | `128` | 此注释记录是保留的，不得在 EMF 元文件中使用 |
