...
description: "PngImage 属性。检索图像的透明颜色（如果存在）。此属性对于需要精确处理图像透明区域的应用程序非常有价值，允许开发者访问和操作所使用的特定透明颜色。"
type: docs
weight: 130
url: /zh/net/aspose.imaging.fileformats.png/pngimage/transparentcolor/
---
## PngImage.TransparentColor property

检索图像的透明颜色（如果存在）。此属性对需要精确处理图像内部透明区域的应用程序非常有价值，允许开发者访问并操作所使用的特定透明颜色。
