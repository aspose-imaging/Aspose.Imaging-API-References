...
description: 初始化PngImageaspose.imaging.fileformats.png/pngimage类的新实例
type: docs
weight: 10
...
这个例子展示了如何创建一个指定大小的PNG图像，用纯色填充它并将其保存到文件中。
...
这个例子展示了如何用指定的颜色类型创建指定大小的PNG图像，用纯色填充并保存它到一个文件。
...
此示例显示如何使用指定选项创建 PNG 图像，使用线性渐变颜色填充并将其保存到文件.
