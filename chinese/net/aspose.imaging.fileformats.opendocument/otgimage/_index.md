...
| 姓名 | 描述 |
| --- | --- |
| [OtgImage](otgimage#constructor)(StreamContainer) | 初始化[`OtgImage`](../otgimage)类的新实例。 |
| [OtgImage](otgimage#constructor_1)(StreamContainer, LoadOptions) | 初始化[`OtgImage`](../otgimage)类的新实例。 |
...
| override [BitsPerPixel](../../aspose.imaging.fileformats.opendocument/odimage/bitsperpixel) { get; } | 获取每像素的图像位数。 |
...
| override [IsCached](../../aspose.imaging.fileformats.opendocument/odimage/iscached) { get; } | 获取对象的数据当前是否缓存，不需要读取数据。 |
...
| override [PageExportingAction](../../aspose.imaging.fileformats.opendocument/otgimage/pageexportingaction) { get; set; } | 获取或设置页面导出动作。 请注意，设置该方法执行后会自动释放页面资源。 它将在每个页面保存之前执行。 |
...
| 姓名 | 描述 |
| --- | --- |
| override [CacheData](../../aspose.imaging/vectormultipageimage/cachedata)() | 缓存数据并确保不会从底层 DataStreamSupporter执行额外的数据加载。数据流容器。 |
...
以下代码片段演示了如何转换OTG 图像转 PDF 和其他图像格式。
