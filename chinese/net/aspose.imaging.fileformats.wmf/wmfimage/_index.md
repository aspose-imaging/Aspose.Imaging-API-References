...
| 姓名 | 描述 |
| --- | --- |
| [WmfImage](wmfimage#constructor)() | 初始化[`WmfImage`](../wmfimage)类的新实例。 |
| [WmfImage](wmfimage#constructor_1)(int, int) | 初始化[`WmfImage`](../wmfimage)类的新实例。 |
...
| override [BitsPerPixel](../../aspose.imaging.fileformats.wmf/wmfimage/bitsperpixel) { get; } | 获取每像素的图像位数。 |
...
| override [IsCached](../../aspose.imaging.fileformats.wmf/wmfimage/iscached) { get; } | 获取一个值，该值指示对象的数据当前是否被缓存，并且不需要读取数据。 |
...
| override [CacheData](../../aspose.imaging.fileformats.wmf/wmfimage/cachedata)() | 缓存数据并确保不会从底层 DataStreamSupporter执行额外的数据加载。数据流容器。 |
...
| [GetPostScript](../../aspose.imaging.fileformats.wmf/wmfimage/getpostscript)() | 获取发布脚本。 |
...
以下示例显示如何将压缩图像（*.emz、*.wmz、*.svgz）从at
...
转换为光栅 此示例显示如何加载 WMF 图像从一个文件并使用 WmfRasterizationOptions 将其转换为 SVG。
