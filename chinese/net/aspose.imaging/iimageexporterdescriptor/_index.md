...
description: 表示图像导出器描述符导出器描述符用于克服在内存和多线程问题中包含每个导出器实例 的必要性
type: docs
weight: 9500
...
表示图像导出器描述符。导出器描述符用于克服在内存和多线程问题中包含每个导出器实例 的必要性。
...
| 姓名 | 描述 |
| --- | --- |
| [CanExport](../../aspose.imaging/iimageexporterdescriptor/canexport)(Image, ImageOptionsBase) | 确定图像导出器是否可以将指定图像导出为保存选项指定的指定图像格式。 |
