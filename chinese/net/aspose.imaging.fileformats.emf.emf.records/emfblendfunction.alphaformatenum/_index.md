...
description: 一个结构它指定源像素和目标像素如何被 解释为相对于 alpha 透明度
type: docs
weight: 3280
...
一个结构，它指定源像素和目标像素如何被 解释为相对于 alpha 透明度。
...
| 姓名 | 价值 | 描述 |
| --- | --- | --- |
| NotTransparency | `0` | 源位图中的像素未指定 Alpha 透明度。在这种 情况下，SrcConstantAlpha 值确定源 和目标位图的混合。请注意，在以下等式中 SrcConstantAlpha 除以 255，生成的值在 范围内 0 到 1。 |
