---
title: "EmfCreateMonoBrush.Usage"
second_title: "Aspose.Imaging for .NET API 参考"
description: "EmfCreateMonoBrush 属性。获取或设置一个 32 位无符号整数，指定如何解释 DIB 头部颜色表中的值。此值必须属于 DIBColors 枚举（第 2.1.9 节）。"
type: docs
weight: 40
url: /zh/net/aspose.imaging.fileformats.emf.emf.records/emfcreatemonobrush/usage/
---
## EmfCreateMonoBrush.Usage property

获取或设置一个 32 位无符号整数，指定如何解释 DIB 头部颜色表中的值。此值必须属于 DIBColors 枚举（第 2.1.9 节）。

```csharp
public EmfDibColors Usage { get; set; }
```

### 另请参见

* enum [EmfDibColors](../../../aspose.imaging.fileformats.emf.emf.consts/emfdibcolors/)
* class [EmfCreateMonoBrush](../)
* namespace [Aspose.Imaging.FileFormats.Emf.Emf.Records](../../emfcreatemonobrush/)
* assembly [Aspose.Imaging](../../../)


