...
| 姓名 | 描述 |
| --- | --- |
| [GifOptions](gifoptions#constructor)() | 初始化[`GifOptions`](../gifoptions)类的新实例。 |
| [GifOptions](gifoptions#constructor_1)(GifOptions) | 初始化[`GifOptions`](../gifoptions)类的新实例。 |
...
| [DoPaletteCorrection](../../aspose.imaging.imageoptions/gifoptions/dopalettecorrection) { get; set; } | 获取或设置一个值，指示是否应用调色板校正。 |
...
| [HasTransparentColor](../../aspose.imaging.imageoptions/gifoptions/hastransparentcolor) { get; set; } | 获取或设置GIF图像是否为透明色的值。 |
...
| [MaxDiff](../../aspose.imaging.imageoptions/gifoptions/maxdiff) { get; set; } | 获取或设置最大允许像素差。如果大于零，将使用有损压缩。 最佳有损压缩的推荐值为 80。30 是非常轻的压缩，200 是重的。 当只引入很少的损失时效果最好，并且由于压缩算法的限制，非常高的损失水平不会带来太多的收益。 允许值的范围是 [0, 1000]。 |
...
以下示例展示了如何在不引用特定图像类型的情况下以一般方式将多页矢量图像转换为 GIF 格式。
