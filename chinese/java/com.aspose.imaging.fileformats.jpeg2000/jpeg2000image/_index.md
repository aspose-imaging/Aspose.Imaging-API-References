...
使用我们的 API 高效操作 JPEG2000 (JP2) 图像文件，支持多种每像素位深，并无缝处理包含关键图像信息的 XMP 元数据。具备无损压缩功能，确保在保持文件完整性的同时实现最佳图像质量，使您能够轻松地根据精确规格定制 JP2 图像。
## 构造函数

| 构造函数 | 描述 |
| --- | --- |
| [Jpeg2000Image(String path)](#Jpeg2000Image-java.lang.String-) | 通过使用要加载的图像路径初始化新实例，开始使用 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 类。 |
| [Jpeg2000Image(String path, int bitsPerPixel)](#Jpeg2000Image-java.lang.String-int-) | 通过同时提供文件路径和所需的每像素位深参数创建新实例，轻松入门使用 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 类。 |
...
| [Jpeg2000Image(InputStream stream, int bitsPerPixel)](#Jpeg2000Image-java.io.InputStream-int-) | 使用流加载图像并提供每像素位深参数，初始化 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 类的新实例。 |
| [Jpeg2000Image(int width, int height)](#Jpeg2000Image-int-int-) | 创建 [Jpeg2000Image] 类的新实例，指定宽度和高度参数。 |
| [Jpeg2000Image(int width, int height, Jpeg2000Options options)](#Jpeg2000Image-int-int-com.aspose.imaging.imageoptions.Jpeg2000Options-) | 实例化一个新的 [Jpeg2000Image] 对象，提供宽度、高度和图像选项参数。 |
| [Jpeg2000Image(int width, int height, int bitsCount)](#Jpeg2000Image-int-int-int-) | 创建 [Jpeg2000Image] 类的新实例，使用宽度、高度和位计数参数。 |
| [Jpeg2000Image(RasterImage image)](#Jpeg2000Image-com.aspose.imaging.RasterImage-) | 实例化一个新的 [Jpeg2000Image] 类，使用栅格图像。 |
| [Jpeg2000Image(RasterImage rasterImage, int bitsPerPixel)](#Jpeg2000Image-com.aspose.imaging.RasterImage-int-) | 初始化一个全新的 [Jpeg2000Image] 实例，使用栅格图像和每像素位数参数。 |
## 方法
...
| [getRawLineSize()](#getRawLineSize--) | 此属性检索原始图像数据单行的大小（字节）。 |
| [getWidth()](#getWidth--) | 此属性返回图像的宽度（像素）。 |
| [getHeight()](#getHeight--) | 此属性检索图像的高度（像素）。 |
| [getBitsPerPixel()](#getBitsPerPixel--) | 此属性返回图像的深度，以每像素位数（bpp）计量。 |
| [getHorizontalResolution()](#getHorizontalResolution--) | 此属性允许您检索或修改 [RasterImage] 的水平分辨率，以每英寸像素数 (PPI) 为单位。 |
| [setHorizontalResolution(double value)](#setHorizontalResolution-double-) | 此属性允许您检索或修改 [RasterImage] 的水平分辨率，以每英寸像素数 (PPI) 为单位。 |
| [getVerticalResolution()](#getVerticalResolution--) | 此属性提供对 [RasterImage] 垂直分辨率的访问，以每英寸像素数 (PPI) 为单位。 |
| [setVerticalResolution(double value)](#setVerticalResolution-double-) | 此属性提供对 [RasterImage] 垂直分辨率的访问，以每英寸像素数 (PPI) 为单位。 |
...
通过使用要加载的图像路径初始化新实例，开始使用 [Jpeg2000Image] 类。此构造函数实现对 JPEG2000 图像的轻松访问，简化加载和处理图像文件的过程。提供文件路径后，您即可在应用程序中快速开始处理和操作 JPEG2000 图像。
...
通过使用文件路径和所需的每像素位数参数创建新实例，轻松入门 [Jpeg2000Image] 类。此构造函数可对图像加载过程进行微调，确保兼容各种图像格式和质量设置。凭借此灵活性，您可以根据具体需求高效管理和操作 JPEG2000 图像。
...
使用流加载图像并提供每像素位数参数来初始化 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 类的新实例。此构造函数通过允许您指定图像数据源和所需的每像素位数，提供了灵活性，从而对图像加载过程进行更精细的控制。
...
实例化一个新的 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 对象，提供宽度、高度和图像选项参数。此构造函数允许创建具有特定尺寸和附加选项的 JPEG2000 图像，为图像生成提供灵活性。
...
使用栅格图像实例化一个新的 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 类。此构造函数便于从现有栅格图像创建 JPEG2000 图像，实现不同图像格式之间的无缝集成和转换。
...
使用栅格图像和每像素位数参数初始化全新的 [Jpeg2000Image](../../com.aspose.imaging.fileformats.jpeg2000/jpeg2000image) 实例。此构造函数能够对生成的 JPEG2000 图像的质量和大小进行精确控制，适用于定制至关重要的场景。
...
此属性检索图像的原始数据格式。它提供像素数据在内存中的存储方式信息。使用此属性可了解图像的底层数据格式，这对于颜色转换、压缩或解压缩等各种图像处理操作至关重要。
...
此属性检索原始图像数据单行的字节大小。它指示图像原始数据格式中单行像素占用的内存量。了解原始行大小对于内存分配、数据操作以及对单行图像进行处理的图像算法等任务至关重要。
...
此属性检索图像的像素高度。它是了解图像垂直尺寸的关键信息，有助于包括缩放、裁剪和渲染在内的各种图像操作。访问此属性可让用户确定图像的垂直大小，从而在应用程序中实现精确的布局和显示。
...
此属性返回图像的深度，以每像素位数 (bpp) 为单位。它指示每个像素存储的颜色信息量。了解图像深度对于确定图像的色彩保真度和质量至关重要。凭借此信息，用户可以评估图像细节层次和颜色丰富度。
...
此属性允许您检索或修改 [RasterImage](../../com.aspose.imaging/rasterimage) 的水平分辨率，单位为每英寸像素数 (PPI)。调整此分辨率会影响图像在打印或显示时的尺寸和质量。通过设置水平分辨率，用户可以针对特定输出设备或应用程序优化图像，确保获得最佳视觉效果。
...
此属性提供对 [RasterImage](../../com.aspose.imaging/rasterimage) 垂直分辨率的访问，单位为每英寸像素数 (PPI)。修改此分辨率会影响图像在打印或显示时的质量和尺寸。通过调整垂直分辨率，用户可以针对不同输出设备或应用程序优化图像，确保最佳的视觉呈现。
...
此属性允许检索或更新与图像关联的注释。注释提供关于图像内容的额外信息，如标注、描述或元数据。修改这些注释有助于对图像进行组织和分类，并向观看者或用户传达重要细节。
