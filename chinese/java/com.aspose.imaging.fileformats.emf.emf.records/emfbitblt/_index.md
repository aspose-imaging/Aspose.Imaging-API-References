...
description: "EMR_BITBLT 记录指定将像素块从源位图传输到目标矩形，可选地结合刷子图案，依据指定的光栅操作。"
type: docs
weight: 16
...
EMR\_BITBLT 记录指定将像素从源位图块传输到目标矩形的操作，可选地与画刷图案结合，依据指定的光栅操作。
## 构造函数
...
获取或设置一个 32 位无符号整数，指定光栅操作码。该码定义如何将源矩形的颜色数据与目标矩形的颜色数据以及可选的刷子图案组合，以实现最终颜色。
