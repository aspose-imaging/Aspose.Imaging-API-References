...
| 方法 | 描述 |
| --- | --- |
| [getVersion()](#getVersion--) | 获取或设置一个 16 位无符号整数，指定 WMF 元文件版本（即对设备无关位图（DIB）支持的程度），来源于 WMF MetafileVersion 枚举（[MS-WMF] 第 2.1.1.19 节）。 |
| [setVersion(short value)](#setVersion-short-) | 获取或设置一个 16 位无符号整数，指定 WMF 元文件版本（即对设备无关位图（DIB）支持的程度），来源于 WMF MetafileVersion 枚举（[MS-WMF] 第 2.1.1.19 节）。 |
| [getChecksum()](#getChecksum--) | 获取或设置一个 32 位无符号整数，指定此记录的校验和。 |
| [setChecksum(int value)](#setChecksum-int-) | 获取或设置一个 32 位无符号整数，指定此记录的校验和。 |
| [getFlags()](#getFlags--) | 获取或设置一个 32 位值，必须为 0x00000000，且必须被忽略。 |
| [setFlags(int value)](#setFlags-int-) | 获取或设置一个 32 位值，必须为 0x00000000，且必须被忽略。 |
| [getWinMetafileSize()](#getWinMetafileSize--) | 获取或设置一个 32 位无符号整数，指定 WinMetafile 字段中 WMF 元文件的大小（以字节为单位）。 |
| [setWinMetafileSize(int value)](#setWinMetafileSize-int-) | 获取或设置一个 32 位无符号整数，指定 WinMetafile 字段中 WMF 元文件的大小（以字节为单位）。 |
...
获取或设置一个 16 位无符号整数，指定 WMF 元文件版本（即对设备无关位图（DIB）支持的程度），来源于 WMF MetafileVersion 枚举（[MS-WMF] 第 2.1.1.19 节）。
...
获取或设置一个 32 位值，必须为 0x00000000，且必须被忽略。
...
获取或设置一个 32 位无符号整数，指定 WinMetafile 字段中 WMF 元文件的大小（以字节为单位）。
