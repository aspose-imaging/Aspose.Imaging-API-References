...
description: "EMR_STRETCHBLT 记录指定了从源位图到目标矩形的像素块传输，可选地结合刷子图案，根据指定的光栅操作进行，必要时将输出拉伸或压缩以适应目标的尺寸。"
type: docs
weight: 149
...
该 EMR\_STRETCHBLT 记录指定将像素块从源位图传输到目标矩形，可选地与画刷图案结合，根据指定的光栅操作进行，如果需要，拉伸或压缩输出以适应目标的尺寸。
## 构造函数
...
获取或设置指定光栅操作码的 32 位无符号整数。此代码定义了如何将源矩形的颜色数据与目标矩形的颜色数据以及可选的刷子图案组合，以获得最终颜色。
...
获取或设置包含源位图的缓冲区，该缓冲区不需要与 EMR\_STRETCHBLT 记录的固定部分连续。因此，缓冲区中标记为 \"UndefinedSpace\" 的字段是可选的，必须被忽略。
