...
description: "EMR_TRANSPARENTBLT 记录指定将像素块从源位图传输到目标矩形的操作，将指定颜色视为透明，并在必要时拉伸或压缩输出以适应目标的尺寸。"
type: docs
weight: 154
...
该 EMR\_TRANSPARENTBLT 记录指定将像素块从源位图传输到目标矩形，将指定颜色视为透明，如果需要，拉伸或压缩输出以适应目标的尺寸。
## 构造函数
...
| [getTransparentArgb32Color()](#getTransparentArgb32Color--) | 获取或设置 WMF ColorRef 对象（[MS-WMF] 第 2.2.2.8 节），该对象指定源位图中应视为透明的颜色。 |
| [setTransparentArgb32Color(int value)](#setTransparentArgb32Color-int-) | 获取或设置 WMF ColorRef 对象（[MS-WMF] 第 2.2.2.8 节），该对象指定源位图中应视为透明的颜色。 |
...
| [getSrcBkArgb32Color()](#getSrcBkArgb32Color--) | 获取或设置 WMF ColorRef 对象，指定源位图的背景颜色。 |
| [setSrcBkArgb32Color(int value)](#setSrcBkArgb32Color-int-) | 获取或设置 WMF ColorRef 对象，指定源位图的背景颜色。 |
...
获取或设置 WMF ColorRef 对象（[MS-WMF] 第 2.2.2.8 节），该对象指定源位图中应视为透明的颜色。
...
获取或设置 WMF ColorRef 对象，指定源位图的背景颜色。
...
获取或设置包含源位图的缓冲区，该缓冲区不需要与 EMR\_TRANSPARENTBLT 记录的固定部分连续。因此，缓冲区中标记为 "UndefinedSpace" 的字段是可选的，必须被忽略。
