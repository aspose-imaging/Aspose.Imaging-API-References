...
EMR_HEADER 记录类型定义 EMF 元文件的起始点，并指定创建该元文件图像的设备属性。标题记录中的信息使 EMF 元文件能够独立于任何特定输出设备。Size 字段的值可用于区分本节前面列出的不同 EMR_HEADER 记录类型。共有三种可能的标题：基础标题，即 EmfMetafileHeader 记录。此标题的固定大小部分为 88 字节，包含一个 Header 对象。第一扩展标题，即 EmfMetafileHeaderExtension1 记录。此标题的固定大小部分为 100 字节，包含一个 Header 对象和一个 HeaderExtension1 对象（第 2.2.10 节）。第二扩展标题，即 EmfMetafileHeaderExtension2 记录。此标题的固定大小部分为 108 字节，包含一个 Header 对象、一个 HeaderExtension1 对象和一个 HeaderExtension2 对象（第 2.2.11 节）。
## 构造函数
...
| 方法 | 描述 |
| --- | --- |
| [getEmfHeader()](#getEmfHeader--) | 获取一个 Header 对象（第 2.2.9 节），该对象包含有关元文件内容和结构的信息 |
| [setEmfHeader(EmfHeaderObject value)](#setEmfHeader-com.aspose.imaging.fileformats.emf.emf.objects.EmfHeaderObject-) | 设置一个 Header 对象（第 2.2.9 节），该对象包含有关元文件内容和结构的信息 |
...
| [getEmfDescriptionBuffer()](#getEmfDescriptionBuffer--) | 获取 EMF 描述缓冲区：一个可选的字节数组，包含 EMF 描述字符串，该字符串不必与 EmfMetafileHeader 记录的固定部分连续。 |
| [setEmfDescriptionBuffer(byte[] value)](#setEmfDescriptionBuffer-byte---) | 设置 EMF 描述缓冲区：一个可选的字节数组，包含 EMF 描述字符串，该字符串不必与 EmfMetafileHeader 记录的固定部分连续。 |
...
获取一个 Header 对象（第 2.2.9 节），该对象包含有关元文件内容和结构的信息
...
设置一个 Header 对象（第 2.2.9 节），该对象包含有关元文件内容和结构的信息
...
获取一个可选的字节数组，其中包含 EMF 标头记录的其余部分。此字段的大小必须是 4 字节的倍数。
...
设置一个可选的字节数组，其中包含 EMF 标头记录的其余部分。此字段的大小必须是 4 字节的倍数。
...
获取 EMF 描述缓冲区：一个可选的字节数组，包含 EMF 描述字符串，该字符串不必与 EmfMetafileHeader 记录的固定部分连续。因此，此缓冲区中标记为 "UndefinedSpace" 的字段是可选的，必须被忽略。
...
设置 EMF 描述缓冲区：一个可选的字节数组，包含 EMF 描述字符串，该字符串不必与 EmfMetafileHeader 记录的固定部分连续。因此，此缓冲区中标记为 "UndefinedSpace" 的字段是可选的，必须被忽略。
