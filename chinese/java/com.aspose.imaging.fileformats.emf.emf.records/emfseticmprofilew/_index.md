...
该 EMR\_SETICMPROFILEW 记录指定用于图形输出的、文件名由 Unicode 字符组成的颜色配置文件。
## 构造函数
...
| [getCbData()](#getCbData--) | 获取或设置一个 32 位无符号整数，指定（如果附加）颜色配置文件数据的大小。 |
| [setCbData(int value)](#setCbData-int-) | 获取或设置一个 32 位无符号整数，指定（如果附加）颜色配置文件数据的大小。 |
...
获取或设置一个 32 位无符号整数，指定（如果附加）颜色配置文件数据的大小。
