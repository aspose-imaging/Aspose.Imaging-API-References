...
EMR\_SETICMPROFILEA 记录指定用于图形输出的、文件名由 ASCII 字符组成的颜色配置文件。
## 构造函数
...
| [getCbData()](#getCbData--) | 获取或设置指定颜色配置文件数据大小的 32 位无符号整数（如果它包含在 Data 字段中）。 |
| [setCbData(int value)](#setCbData-int-) | 获取或设置指定颜色配置文件数据大小的 32 位无符号整数（如果它包含在 Data 字段中）。 |
| [getData()](#getData--) | 获取或设置大小为 (cbName + cbData) 字节的数组，该数组指定所需颜色配置文件的 ASCII 名称和原始数据。 |
| [setData(byte[] value)](#setData-byte---) | 获取或设置大小为 (cbName + cbData) 字节的数组，该数组指定所需颜色配置文件的 ASCII 名称和原始数据。 |
...
获取或设置指定颜色配置文件数据大小的 32 位无符号整数（如果它包含在 Data 字段中）。
...
获取或设置大小为 (cbName + cbData) 字节的数组，该数组指定所需颜色配置文件的 ASCII 名称和原始数据。
