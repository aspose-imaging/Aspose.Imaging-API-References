...
description: "针对 Corel Metafile Exchange CMX 矢量图像格式并支持元数据描述的 API 是为使用 CMX 文件的开发者提供的综合解决方案。"
type: docs
weight: 10
...
| 构造函数 | 描述 |
| --- | --- |
| [CmxImage(StreamContainer streamContainer, LoadOptions loadOptions)](#CmxImage-com.aspose.imaging.StreamContainer-com.aspose.imaging.LoadOptions-) | 通过使用 streamContainer 和 loadOptions 参数初始化新实例，即可无缝开始使用 [CmxImage](../../com.aspose.imaging.fileformats.cmx/cmximage) 类。 |
## 方法
...
| [getDefaultPage()](#getDefaultPage--) | 使用此直观属性，轻松检索图像的默认页面。 |
| [isCached()](#isCached--) | 确定对象的数据当前是否已缓存，从而消除读取数据的需求。 |
| [getWidthF()](#getWidthF--) | 使用此直观属性，获取对象以英寸为单位的宽度。 |
| [getHeightF()](#getHeightF--) | 使用此用户友好的属性，轻松获取对象以英寸为单位的高度。 |
| [getDocument()](#getDocument--) | 使用此直观属性，轻松获取 CMX 文档。 |
| [getCmxPage()](#getCmxPage--) | 使用此直观属性，轻松获取图像的 CMX 页面。 |
| [getPageCount()](#getPageCount--) | 使用此直观属性，获取图像的总页数。 |
| [getPages()](#getPages--) | 使用此直观属性，顺畅获取图像的页面。 |
| [cacheData()](#cacheData--) | 使用此便捷方法缓存数据，以防止从底层源 [DataStreamSupporter](../../com.aspose.imaging/datastreamsupporter) 进行额外加载。 |
| [setPalette(IColorPalette palette, boolean updateColors)](#setPalette-com.aspose.imaging.IColorPalette-boolean-) | 使用此直观方法自定义图像的颜色调色板。 |
...
通过使用 streamContainer 和 loadOptions 参数初始化新实例，您可以无缝开始使用 [CmxImage](../../com.aspose.imaging.fileformats.cmx/cmximage) 类。此方式非常适合希望以便捷方式从各种数据源加载 CMX 图像并根据需要自定义加载过程的开发者。
...
使用此便捷方法缓存数据，以防止从底层来源 [DataStreamSupporter](../../com.aspose.imaging/datastreamsupporter) 进行额外加载。适用于希望通过预加载数据来优化性能的开发者，确保在应用程序中更快的访问和更流畅的运行。
