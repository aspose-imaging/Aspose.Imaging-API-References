...
该 RecordType 枚举定义在 EMF+ 元文件中使用的记录类型。
## 字段
...
| [EmfPlusGetDC](#EmfPlusGetDC) | 此记录指定元文件中后续遇到的 EMF 记录应当被处理。 |
| [EmfPlusMultiFormatStart](#EmfPlusMultiFormatStart) | 此记录为保留记录，禁止使用。 |
| [EmfPlusMultiFormatSection](#EmfPlusMultiFormatSection) | 此记录为保留记录，禁止使用。 |
| [EmfPlusMultiFormatEnd](#EmfPlusMultiFormatEnd) | 此记录为保留记录，禁止使用。 |
| [EmfPlusObject](#EmfPlusObject) | 此记录指定用于图形操作的对象。 |
| [EmfPlusClear](#EmfPlusClear) | 此记录清除输出 `coordinate space` 并使用指定的背景颜色和透明度进行初始化。 |
| [EmfPlusFillRects](#EmfPlusFillRects) | 此记录定义使用指定画刷填充一系列矩形的内部方式。 |
| [EmfPlusDrawRects](#EmfPlusDrawRects) | 此记录定义绘制一系列矩形的笔画。 |
| [EmfPlusFillPolygon](#EmfPlusFillPolygon) | 此记录定义使用指定画刷填充多边形内部的数据。 |
| [EmfPlusDrawLines](#EmfPlusDrawLines) | 此记录定义绘制一系列相连线段的笔画。 |
| [EmfPlusFillEllipse](#EmfPlusFillEllipse) | 此记录定义使用指定画刷填充椭圆内部的方式。 |
| [EmfPlusDrawEllipse](#EmfPlusDrawEllipse) | 此记录定义绘制椭圆的笔画。 |
| [EmfPlusFillPie](#EmfPlusFillPie) | 此记录定义了如何使用指定的画刷填充椭圆内部的一个区域段。 |
| [EmfPlusDrawPie](#EmfPlusDrawPie) | 此记录定义了绘制椭圆段的笔画。 |
| [EmfPlusDrawArc](#EmfPlusDrawArc) | 该记录定义了绘制椭圆弧的笔画。 |
| [EmfPlusFillRegion](#EmfPlusFillRegion) | 此记录定义了如何使用指定的画刷填充区域的内部。 |
| [EmfPlusFillPath](#EmfPlusFillPath) | 该记录定义了如何使用指定的画刷填充图形路径中定义的图形的内部。 |
| [EmfPlusDrawPath](#EmfPlusDrawPath) | 该记录定义了在图形路径中绘制图形的笔画。 |
| [EmfPlusFillClosedCurve](#EmfPlusFillClosedCurve) | 此记录定义了如何使用指定的画刷填充闭合基数样条的内部。 |
| [EmfPlusDrawClosedCurve](#EmfPlusDrawClosedCurve) | 此记录定义了绘制闭合基数样条的笔和笔画。 |
| [EmfPlusDrawCurve](#EmfPlusDrawCurve) | 此记录定义了绘制基数样条的笔画。 |
| [EmfPlusDrawBeziers](#EmfPlusDrawBeziers) | 此记录定义了绘制贝塞尔样条的笔画。 |
| [EmfPlusDrawImage](#EmfPlusDrawImage) | 此记录定义了一个缩放的 [EmfPlusImage](../../com.aspose.imaging.fileformats.emf.emfplus.objects/emfplusimage) 对象（第 2.2.1.4 节）。 |
| [EmfPlusDrawImagePoints](#EmfPlusDrawImagePoints) | 此记录定义了在平行四边形内的缩放 EmfPlusImage 对象。 |
| [EmfPlusDrawString](#EmfPlusDrawString) | 此记录定义了基于字体、布局矩形和格式的文本字符串。 |
| [EmfPlusSetRenderingOrigin](#EmfPlusSetRenderingOrigin) | 此记录定义了渲染原点到指定的水平和垂直坐标。 |
| [EmfPlusSetAntiAliasMode](#EmfPlusSetAntiAliasMode) | 此记录定义了是否启用或禁用文本抗锯齿。 |
| [EmfPlusSetTextRenderingHint](#EmfPlusSetTextRenderingHint) | 此记录定义了用于渲染文本的过程。 |
...
| [EmfPlusSetCompositingMode](#EmfPlusSetCompositingMode) | 此记录根据 alpha 混合的状态定义合成模式，该模式指定源颜色如何与背景颜色组合。 |
| [EmfPlusSetCompositingQuality](#EmfPlusSetCompositingQuality) | 此记录定义了合成质量，描述了从多个对象创建复合图像所需的质量水平。 |
| [EmfPlusSave](#EmfPlusSave) | 此记录将由指定索引标识的图形状态保存在已保存图形状态的堆栈上。 |
...
| [EmfPlusEndContainer](#EmfPlusEndContainer) | 此记录关闭先前由 begin container 操作打开的图形状态容器。 |
| [EmfPlusSetWorldTransform](#EmfPlusSetWorldTransform) | 此记录根据指定的变换矩阵定义 playback device\_context 中的当前世界空间变换。 |
...
| [EmfPlusTranslateWorldTransform](#EmfPlusTranslateWorldTransform) | 此记录根据指定的水平和垂直距离对当前世界空间应用平移变换。 |
| [EmfPlusScaleWorldTransform](#EmfPlusScaleWorldTransform) | 此记录根据指定的水平和垂直缩放因子对当前世界空间应用缩放变换。 |
...
| [EmfPlusStrokeFillPath](#EmfPlusStrokeFillPath) | 此记录关闭路径中所有未闭合的图形，使用当前笔描绘路径轮廓，并使用当前画笔填充其内部。 |
...
此记录指定元文件中 EMF+ 数据的开始。它必须嵌入在第一个 EMF 记录之后的 [EmfMetafileHeader](../../com.aspose.imaging.fileformats.emf.emf.records/emfmetafileheader) 记录中（[MS-EMF] 第 2.3.4.2 节记录）。
...
此记录指定在元文件中遇到的后续 EMF 记录应被处理。当遇到下一个 EMF+ 记录时，EMF 记录的处理将停止。
...
该记录定义如何使用指定的画刷填充图形路径中定义的图形内部。路径是一种对象，定义任意序列的直线、曲线和形状。
...
该记录定义用于绘制图形路径中图形的笔画。路径是一种对象，定义任意序列的直线、曲线和形状。
...
此记录在平行四边形内定义一个缩放的 EmfPlusImage 对象。图像可以由位图或元文件数据组成。
...
此记录将渲染原点定义为指定的水平和垂直坐标。这适用于交叉线画刷以及每像素 8 位和 16 位的抖动模式。
...
此记录定义是否启用或禁用文本抗锯齿。文本抗锯齿是一种在输出表面上绘制字符字形时使线条和边缘更平滑的方法。
...
此记录根据 alpha 混合的状态定义合成模式，该模式指定源颜色如何与背景颜色组合。
...
此记录定义了合成质量，描述了从多个对象创建复合图像所需的质量水平。
...
此记录将由指定索引标识的图形状态保存到已保存图形状态的堆栈上。每个堆栈索引对应特定的已保存状态，且该索引用于[EmfPlusRestore](../../com.aspose.imaging.fileformats.emf.emfplus.records/emfplusrestore)记录（第 2.3.7.4 节）以恢复状态。
...
此记录从已保存图形状态的堆栈中恢复由指定索引标识的图形状态。每个堆栈索引对应特定的已保存状态，且该索引由[EmfPlusSave](../../com.aspose.imaging.fileformats.emf.emfplus.records/emfplussave)记录（第 2.3.7.5 节）定义以保存状态。
...
此记录关闭先前由 begin container 操作打开的图形状态容器。
...
此记录根据指定的变换矩阵定义 playback device\_context 中的当前世界空间变换。
...
此记录根据指定的水平和垂直缩放因子对当前世界空间应用缩放变换。
