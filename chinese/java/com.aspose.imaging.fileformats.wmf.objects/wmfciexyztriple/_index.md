---
title: "WmfCieXyzTriple"
second_title: "Aspose.Imaging for Java API 参考文档"
description: "CIEXYZTriple 对象定义有关 CIEXYZTriple 颜色对象的信息。"
type: docs
weight: 19
url: /zh/java/com.aspose.imaging.fileformats.wmf.objects/wmfciexyztriple/
---
**Inheritance:**
java.lang.Object
```
public class WmfCieXyzTriple
```

CIEXYZTriple 对象定义有关 CIEXYZTriple 颜色对象的信息。
## 构造函数

| 构造函数 | 描述 |
| --- | --- |
| [WmfCieXyzTriple()](#WmfCieXyzTriple--) |  |
## 方法

| 方法 | 描述 |
| --- | --- |
| [getCiexyzRed()](#getCiexyzRed--) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义红色色度值。 |
| [setCiexyzRed(WmfCieXyz value)](#setCiexyzRed-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义红色色度值。 |
| [getCiexyzGreen()](#getCiexyzGreen--) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义绿色色度值。 |
| [setCiexyzGreen(WmfCieXyz value)](#setCiexyzGreen-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义绿色色度值。 |
| [getCiexyzBlue()](#getCiexyzBlue--) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义蓝色色度值。 |
| [setCiexyzBlue(WmfCieXyz value)](#setCiexyzBlue-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-) | 获取或设置一个 96 位 CIEXYZ 对象，用于定义蓝色色度值。 |
### WmfCieXyzTriple() {#WmfCieXyzTriple--}
```
public WmfCieXyzTriple()
```


### getCiexyzRed() {#getCiexyzRed--}
```
public WmfCieXyz getCiexyzRed()
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义红色色度值。

**Returns:**
[WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz)
### setCiexyzRed(WmfCieXyz value) {#setCiexyzRed-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-}
```
public void setCiexyzRed(WmfCieXyz value)
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义红色色度值。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | [WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz) |  |

### getCiexyzGreen() {#getCiexyzGreen--}
```
public WmfCieXyz getCiexyzGreen()
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义绿色色度值。

**Returns:**
[WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz)
### setCiexyzGreen(WmfCieXyz value) {#setCiexyzGreen-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-}
```
public void setCiexyzGreen(WmfCieXyz value)
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义绿色色度值。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | [WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz) |  |

### getCiexyzBlue() {#getCiexyzBlue--}
```
public WmfCieXyz getCiexyzBlue()
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义蓝色色度值。

**Returns:**
[WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz)
### setCiexyzBlue(WmfCieXyz value) {#setCiexyzBlue-com.aspose.imaging.fileformats.wmf.objects.WmfCieXyz-}
```
public void setCiexyzBlue(WmfCieXyz value)
```


获取或设置一个 96 位 CIEXYZ 对象，用于定义蓝色色度值。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | [WmfCieXyz](../../com.aspose.imaging.fileformats.wmf.objects/wmfciexyz) |  |

