...
META\_SETMAPPERFLAGS 记录定义字体映射器在将逻辑字体映射到物理字体时使用的算法。
## 构造函数
...
| 方法 | 描述 |
| --- | --- |
| [getMapperValues()](#getMapperValues--) | 获取或设置映射器值。 |
| [setMapperValues(int value)](#setMapperValues-int-) | 获取或设置映射器值。 |
### WmfSetMapperFlags() {#WmfSetMapperFlags--}
```
...
值：字体映射器尝试将字体宽高比匹配到当前设备的宽高比。如果设置了位零，映射器仅选择匹配的字体。
