...
| 方法 | 描述 |
| --- | --- |
| [getHeaderSize()](#getHeaderSize--) | 获取或设置一个 32 位无符号整数，定义此对象的大小（字节）。 |
| [setHeaderSize(int value)](#setHeaderSize-int-) | 获取或设置一个 32 位无符号整数，定义此对象的大小（字节）。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | int | 一个 16 位无符号整数，定义目标设备的 `planes` 数量。该值必须为 0x0001。 |
...
获取或设置一个 16 位无符号整数，定义目标设备的 `planes` 数量。该值必须为 0x0001。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | 短 | 一个 16 位无符号整数，定义目标设备的 `planes` 数量。该值必须为 \* 0x0001。 |
...
获取或设置一个 16 位无符号整数，定义每个像素的格式以及 DIB 中的最大颜色数。该值必须属于 `BitCount` 枚举（第 2.1.1.3 节）。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | 短 | 一个 16 位无符号整数，定义每个像素的格式以及 DIB 中的最大颜色数。 |
