...
description: "BitmapInfoHeader 对象包含有关设备无关位图 DIB 的尺寸和颜色格式的信息。"
type: docs
weight: 16
...
| [getXPelsPerMeter()](#getXPelsPerMeter--) | 获取或设置一个 32 位有符号整数，定义 DIB 目标设备的水平分辨率（像素每米）。 |
| [setXPelsPerMeter(int value)](#setXPelsPerMeter-int-) | 获取或设置一个 32 位有符号整数，定义 DIB 目标设备的水平分辨率（像素每米）。 |
| [getYPelsPerMeter()](#getYPelsPerMeter--) | 获取或设置一个 32 位有符号整数，定义 DIB 目标设备的垂直分辨率（像素每米）。 |
| [setYPelsPerMeter(int value)](#setYPelsPerMeter-int-) | 获取或设置一个 32 位有符号整数，定义 DIB 目标设备的垂直分辨率（像素每米）。 |
| [getColorUsed()](#getColorUsed--) | 获取或设置一个 32 位无符号整数，指定 DIB 使用的颜色表中的索引数量，规则如下：如果该值为零，DIB 使用与 BitCount 值对应的最大颜色数。 |
| [setColorUsed(int value)](#setColorUsed-int-) | 获取或设置一个 32 位无符号整数，指定 DIB 使用的颜色表中的索引数量，规则如下：如果该值为零，DIB 使用与 BitCount 值对应的最大颜色数。 |
...
获取或设置一个 32 位无符号整数，定义图像的大小（以字节为单位）。如果 Compression 值为 BI\_RGB，则该值应为零并必须被忽略。如果 Compression 值为 BI\_JPEG 或 BI\_PNG，则该值必须分别指定 JPEG 或 PNG 图像缓冲区的大小。
