...
description: "LogColorSpace 对象为回放设备上下文指定逻辑颜色空间，该名称可以是 ASCII 字符形式的颜色配置文件名称。"
type: docs
weight: 44
...
该 LogColorSpace 对象为回放设备上下文指定逻辑颜色空间，该空间可以是 ASCII 字符的颜色配置文件名称。

Endpoints、GammaRed、GammaGreen 和 GammaBlue 字段用于指定逻辑颜色空间。Endpoints 字段是一个 CIEXYZTriple 对象，包含颜色空间的 RGB 端点的 x、y、z 值。三刺激值 X、Y、Z 与色度值 x、y、z 之间的关系表示如下：x = X/(X+Y+Z) y = Y/(X+Y+Z) z = Z/(X+Y+Z)。GammaRed、GammaGreen 和 GammaBlue 字段包含 \"8.8 fixed point\" 格式的值，这是一种表示非整数数字的技术。每个值由一个零扩展的 8 位整数部分和一个 8 位小数部分组成，合计 16 位左移 8 位。因此，在 32 位中，实际值 N.F 表示为 00000000nnnnnnnnffffffff00000000，其中 \"nnnnnnnn\" 和 \"ffffffff\" 分别是 N 和 F 的二进制表示。例如，对于实数 10.5，nnnnnnnn 为 00001010（二进制 10），ffffffff 为 00000101（二进制 5），完整的 32 位二进制值为 00000000000010100000010100000000，即十六进制值 0x0A50。
## 构造函数
...
| [getFilename()](#getFilename--) | 获取或设置一个可选的 ASCII 字符串，用于指定包含颜色配置文件的文件名。 |
| [setFilename(String value)](#setFilename-java.lang.String-) | 获取或设置一个可选的 ASCII 字符串，用于指定包含颜色配置文件的文件名。 |
### WmfLogColorSpace() {#WmfLogColorSpace--}
```
...
获取或设置一个可选的 ASCII 字符串，用于指定包含颜色配置文件的文件名。如果指定了文件名且 `ColorSpaceType` 字段被设置为 LCS\\_CALIBRATED\\_RGB，则应忽略此结构的其他字段。
