...
description: "使用我们的 API 轻松创建高质量的 JPEG 图像，提供可调节的压缩级别，以在不影响图像质量的情况下优化存储大小。"
type: docs
weight: 26
...
使用我们的 API 轻松创建高质量的 JPEG 图像，提供可调节的压缩级别，以在不影响图像质量的情况下优化存储大小。受益于对多种压缩类型、近乎无损编码、RGB 和 CMYK 色彩配置文件以及 EXIF、JFIF 图像数据和 XMP 容器的支持，确保为您的图像创建需求提供多功能且可定制的选项。
## 构造函数
...
| [setDefaultMemoryAllocationLimit(int value)](#setDefaultMemoryAllocationLimit-int-) | 设置默认的内存分配限制。 |
...
| [setRdOptSettings(RdOptimizerSettings value)](#setRdOptSettings-com.aspose.imaging.imageoptions.RdOptimizerSettings-) | 设置 RD 优化器的设置。 |
...
| [setJpegLsAllowedLossyError(int value)](#setJpegLsAllowedLossyError-int-) | 设置 JPEG-LS 近无损编码的差值界限（来自 JPEG-LS 规范的 NEAR 参数）。 |
...
| [getSampleRoundingMode()](#getSampleRoundingMode--) | 获取将 8 位值适配为 n 位值的采样四舍五入模式。 |
| [setSampleRoundingMode(int value)](#setSampleRoundingMode-int-) | 设置将 8 位值适配为 n 位值的采样四舍五入模式。 |
...
try (com.aspose.imaging.Image image = com.aspose.imaging.Image.load(inputFilePath))
{
    ...
    // 仅导出前两页。实际上，只会栅格化一页，因为 JPEG 不是多页格式。
...
获取无损 JPEG 图像的每通道位数。现在支持每通道 2 到 8 位。
...
设置无损 JPEG 图像的每通道位数。现在支持每通道 2 到 8 位。
...
CMYK JPEG 图像的目标 RGB 颜色配置文件。用于保存图像。必须与 CMYKColorProfile 配对，以实现正确的颜色转换。
...
CMYK JPEG 图像的目标 CMYK 颜色配置文件。用于保存图像。必须与 RGBColorProfile 配对，以实现正确的颜色转换。
...
设置 JPEG-LS 近无损编码的差值界限（来自 JPEG-LS 规范的 NEAR 参数）。
...
获取样本四舍五入模式，以将 8 位值适配为 n 位值。`P:JpegOptions.BitsPerChannel`
