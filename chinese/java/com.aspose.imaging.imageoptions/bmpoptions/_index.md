...
description: "BMP 和 DIB 栅格图像格式创建选项的 API 为开发者提供了一个多功能工具集，用于生成自定义的 Bitmap BMP 和 Device Independent Bitmap DIB 图像。"
type: docs
weight: 12
...
BMP 和 DIB 栅格图像格式创建选项的 API 为开发者提供了一个多功能工具集，用于生成自定义的 Bitmap (BMP) 和 Device Independent Bitmap (DIB) 图像。使用此 API，您可以精确定义图像特性，例如每像素位数、压缩级别和压缩类型，以满足特定需求。功能丰富的 API 使开发者能够轻松灵活地创建高质量、定制化的栅格图像，以适用于各种应用。
## 构造函数
...
| 方法 | 描述 |
| --- | --- |
| [getBitsPerPixel()](#getBitsPerPixel--) | 获取或设置图像每像素位数计数。 |
| [setBitsPerPixel(int value)](#setBitsPerPixel-int-) | 获取或设置图像每像素位数计数。 |
...
try (com.aspose.imaging.Image image = com.aspose.imaging.Image.load(inputFilePath))
{
    ...
    // 仅导出前两页。事实上，由于 BMP 不是多页格式，只有一页会被光栅化。
...
获取压缩类型。默认压缩类型是 [BitmapCompression.Bitfields](../../com.aspose.imaging.fileformats.bmp/bitmapcompression\#Bitfields)，它允许保存具有透明度的 [BmpImage](../../com.aspose.imaging.fileformats.bmp/bmpimage)。
...
设置压缩类型。默认压缩类型是 [BitmapCompression.Bitfields](../../com.aspose.imaging.fileformats.bmp/bitmapcompression\#Bitfields)，它允许保存具有透明度的 [BmpImage](../../com.aspose.imaging.fileformats.bmp/bmpimage)。
