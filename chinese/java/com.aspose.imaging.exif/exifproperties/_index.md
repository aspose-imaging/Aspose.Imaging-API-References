...
| [XResolution](#XResolution) | ImageWidth 方向上每个分辨率单位的像素数。 |
| [YResolution](#YResolution) | ImageLength 方向上每个分辨率单位的像素数。 |
...
| [TransferFunction](#TransferFunction) | 图像的传递函数，以表格形式描述。 |
...
| [PrimaryChromaticities](#PrimaryChromaticities) | 图像中三原色的色度。 |
| [YCbCrCoefficients](#YCbCrCoefficients) | 从 RGB 到 YCbCr 图像数据转换的矩阵系数。 |
| [YCbCrSubSampling](#YCbCrSubSampling) | 色度分量相对于亮度分量的采样比例。 |
...
| [PhotographicSensitivity](#PhotographicSensitivity) | 指示相机或输入设备的 ISO 速度和 ISO 量程（依据 ISO 12232）。 |
...
| [SubjectDistance](#SubjectDistance) | 到被摄体的距离，以米为单位。 |
...
| [FlashEnergy](#FlashEnergy) | 指示拍摄图像时的闪光能量，单位为光束烛光功率秒（BCPS）。 |
| [SpatialFrequencyResponse](#SpatialFrequencyResponse) | 此标签记录相机或输入设备的空间频率表和 SFR 值，分别沿图像宽度、图像高度和对角线方向，符合 ISO 12233 的规定。 |
...
| [ExposureIndex](#ExposureIndex) | 指示拍摄图像时相机或输入设备所选的曝光指数。 |
| [SensingMethod](#SensingMethod) | 指示相机或输入设备的图像传感器类型。 |
| [FileSource](#FileSource) | 文件来源。 |
| [SceneType](#SceneType) | 指示场景类型。 |
| [CFAPattern](#CFAPattern) | 指示使用单芯片彩色区域传感器时图像传感器的彩色滤光阵列（CFA）几何图案。 |
...
| [DigitalZoomRatio](#DigitalZoomRatio) | 此标签指示拍摄图像时的数码变焦比率。 |
| [FocalLengthIn35MmFilm](#FocalLengthIn35MmFilm) | 此标签指示在假设使用35mm胶片相机时的等效焦距，单位为毫米。 |
| [SceneCaptureType](#SceneCaptureType) | 此标签指示拍摄场景的类型。 |
| [GainControl](#GainControl) | 此标签指示整体图像增益调整的程度。 |
| [Contrast](#Contrast) | 此标签指示拍摄图像时相机所应用的对比度处理方向。 |
| [Saturation](#Saturation) | 此标签指示拍摄图像时相机所应用的饱和度处理方向。 |
| [Sharpness](#Sharpness) | 此标签指示拍摄图像时相机所应用的锐度处理方向 |
| [DeviceSettingDescription](#DeviceSettingDescription) | 此标签指示特定相机型号的拍摄条件信息。 |
| [SubjectDistanceRange](#SubjectDistanceRange) | 此标签指示到被摄体的距离。 |
| [ImageUniqueID](#ImageUniqueID) | 该图像唯一标识。 |
...
| [GPSAltitude](#GPSAltitude) | 指示基于 GPSAltitudeRef 中参考的海拔。 |
...
| [GPSDestBearingRef](#GPSDestBearingRef) | 指示用于给出指向目的地点方位的参考。 |
| [GPSDestBearing](#GPSDestBearing) | 指示指向目的地点的方位角。 |
| [GPSDestDistanceRef](#GPSDestDistanceRef) | 指示用于表示到目的地点距离的单位。 |
| [GPSDestDistance](#GPSDestDistance) | 指示到目的地点的距离。 |
...
| [LensModel](#LensModel) | 此标签记录镜头\`的型号名称和型号编号 |
| [LensSerialNumber](#LensSerialNumber) | 此标签记录可换镜头的序列号 |
| [LensSpecification](#LensSpecification) | 此标签记录最小焦距、最大焦距、最小焦距下的最小光圈值以及最大焦距下的最小光圈值 |
...
用于图像数据的压缩方案。当主图像采用 JPEG 压缩时，此指定不是必需的，会被省略。
...
记录设备的制造商。这是生成图像的 DSC、扫描仪、视频数字化仪或其他设备的制造商。当该字段留空时，视为未知。
...
设备的型号名称或型号编号。这是生成图像的 DSC、扫描仪、视频数字化仪或其他设备的型号名称或编号。当该字段留空时，视为未知。
...
每像素的组件数。由于本标准适用于 RGB 和 YCbCr 图像，此标签的值设为 3。
...
ImageWidth 方向上每个 ResolutionUnit 的像素数。当图像分辨率未知时，指定为 72 [dpi]。
...
ImageLength 方向上每个 ResolutionUnit 的像素数。指定的值与 XResolution 相同。
...
指示像素组件是以块状还是平面格式记录。如果此字段不存在，则假定 TIFF 的默认值 1（块状）。
...
用于测量 XResolution 和 YResolution 的单位。XResolution 和 YResolution 使用相同的单位。如果图像分辨率未知，指定为 2（英寸）。
...
图像的传递函数，以表格形式描述。通常此标签不是必需的，因为颜色空间已在 ColorSpace 标签的颜色空间信息中指定。
...
此标签记录用于生成图像的相机或图像输入设备的软件或固件的名称和版本。未指定详细格式，但建议遵循以下示例。当该字段留空时，视为未知。
...
此标签记录相机所有者、摄影师或图像创作者的姓名。未指定详细格式，但建议按以下示例编写信息，以便于互操作性。当该字段留空时，视为未知。（例如："Camera owner, John Smith; Photographer, Michael Brown; Image creator, Ken James"）
...
图像白点的色度。通常此标签不是必需的，因为颜色空间已在颜色空间信息 ColorSpace 标签中指定。
...
图像三原色的色度。通常此标签不是必需的，因为颜色空间已在颜色空间信息 ColorSpace 标签中指定。
...
色度分量相对于亮度分量的位置。此字段仅用于 JPEG 压缩数据或未压缩的 YCbCr 数据。TIFF 默认值为 1（居中）；但当 Y:Cb:Cr = 4:2:2 时，本标准建议使用 2（并列）来记录数据，以提高在电视系统上观看时的图像质量。当此字段不存在时，读取器应假定 TIFF 默认值。对于 Y:Cb:Cr = 4:2:0，推荐使用 TIFF 默认（居中）。如果读取器不具备支持两种 YCbCrPositioning 的能力，则应无论该字段的值如何都遵循 TIFF 默认。最好读取器能够支持居中和并列两种定位方式。
...
参考黑点值和参考白点值。TIFF 未给出默认值，但下面的数值在此作为默认值。颜色空间在颜色空间信息标签中声明，默认值为能够提供最佳图像特性和互操作性的数值。
...
版权信息。在本标准中，此标签用于指示摄影师和编辑的版权。它是对声称对图像拥有权利的个人或组织的版权声明。应在此字段中写入包括日期和权利的互操作性版权声明，例如："Copyright, John Smith, 19xx. All rights reserved." 在本标准中，该字段记录摄影师和编辑的版权，分别记录在声明的不同部分。当摄影师和编辑的版权有明确区分时，按摄影师后接编辑的顺序书写，并以 NULL 分隔（由于声明末尾也有一个 NULL，因此共有两个 NULL 代码）。仅提供摄影师版权时，以一个 NULL 代码结束。仅提供编辑版权时，摄影师版权部分为一个空格后跟终止的 NULL 代码，然后给出编辑版权。当字段留空时，视为未知。
...
指示相机或输入设备的 ISO 速度和 ISO 量程（依据 ISO 12232）。
...
此标签记录相机或输入设备的空间频率表和 SFR 值，分别沿图像宽度、图像高度和对角线方向，符合 ISO 12233 的规定。
...
指示场景中主体的位置信息。此标签的值表示主体中心像素相对于左边缘的像素位置，位于根据 Rotation 标签进行旋转处理之前。
...
指示场景类型。如果图像由 DSC 记录，则此标签值应始终设为 1，表示图像是直接拍摄的。
...
指示在使用单芯片彩色区域传感器时图像传感器的颜色滤光阵列 (CFA) 几何图案。该信息并不适用于所有感测方式。
...
此标签指示对图像数据使用了特殊处理，例如针对输出的渲染。当执行特殊处理时，读取器应禁用或尽量减少后续的任何处理。
...
此标签指示拍摄时设置的曝光模式。在自动包围曝光模式下，相机会以不同的曝光设置拍摄同一场景的多帧图像。
...
此标签提供特定相机型号的拍摄条件信息。该标签仅用于在读取器中指示拍摄条件。
...
指示纬度。纬度以三个 RATIONAL 值表示，分别为度、分、秒。如果以度、分、秒表示，典型格式为 dd/1,mm/1,ss/1。当使用度和分且例如分的小数保留两位时，格式为 dd/1,mmmm/100,0/1。
...
指示经度。经度以三个 RATIONAL 值表示，分别为度、分、秒。如果以度、分、秒表示，典型格式为 ddd/1,mm/1,ss/1。当使用度和分且例如分的小数保留两位时，格式为 ddd/1,mmmm/100,0/1。
...
指示用作参考的海拔高度。如果参考为海平面且海拔在海平面以上，则给出 0；如果海拔在海平面以下，则给出 1，并在 GPSAltitude 标签中以绝对值表示海拔。
...
根据 GPSAltitudeRef 的参考指示海拔。海拔以一个 RATIONAL 值表示。参考单位为米。
...
指示时间为 UTC（协调世界时）。TimeStamp 以三个 RATIONAL 值表示，分别为小时、分钟和秒。
...
指示用于测量的 GPS 卫星。此标签可用于描述卫星数量、其 ID 编号、仰角、方位角、信噪比以及其他以 ASCII 记号表示的信息。格式未指定。如果 GPS 接收器无法进行测量，则该标签的值应设为 NULL。
...
指示 GPS DOP（数据精度）。在二维测量时记录 HDOP 值，三维测量时记录 PDOP 值。
...
指示用于给出 GPS 接收器移动方向的参考。'T' 表示真方向，'M' 表示磁方向。
...
指示用于给出拍摄时图像方向的参考。'T' 表示真方向，'M' 表示磁方向。
...
指示目的地点的纬度。纬度以三个有理数值表示，分别为度、分、秒。如果以度、分、秒表示纬度，典型格式为 dd/1,mm/1,ss/1。当使用度和分且例如分的小数部分保留两位小数时，格式为 dd/1,mmmm/100,0/1。
...
指示目的地点的经度。经度以三个有理数值表示，分别为度、分、秒。如果以度、分、秒表示经度，典型格式为 ddd/1,mm/1,ss/1。当使用度和分且例如分的小数部分保留两位小数时，格式为 ddd/1,mmmm/100,0/1。
...
指示用于给出指向目的地点方位的参考。'T' 表示真方向，'M' 表示磁方向。
...
记录用于定位的方法名称的字符字符串。第一个字节指示使用的字符编码，随后是方法名称。
...
对于每个条带，记录该条带的字节偏移。建议选择使条带字节数不超过 64 KBytes。辅助标签。
...
指向 JPEG 压缩缩略图数据起始字节 (SOI) 的偏移。此字段不用于主图像的 JPEG 数据。
...
JPEG 压缩缩略图数据的字节数。此字段不用于主图像的 JPEG 数据。JPEG 缩略图不分段，而是从 SOI 到 EOI 作为连续的 JPEG 位流记录。不应记录 Appn 和 COM 标记。压缩缩略图的记录不得超过 64 KBytes，包括在 APP1 中记录的所有其他数据。
...
指向 Exif IFD 的指针。该互操作性 Exif IFD 结构与 TIFF 中指定的 IFD 相同。但通常情况下，它不像 TIFF 那样包含图像数据。
...
每个条带的行数。当图像被划分为条带时，这是一条带中图像的行数。
...
此标签记录最小焦距、最大焦距、最小焦距下的最小光圈值以及最大焦距下的最小光圈值
