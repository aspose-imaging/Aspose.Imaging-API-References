---
title: "EmfPlusSetInterpolationMode"
second_title: "Aspose.Imaging for Java API 参考文档"
description: "EmfPlusSetInterpolationMode 记录指定图像缩放（包括拉伸和收缩）的执行方式。"
type: docs
weight: 60
url: /zh/java/com.aspose.imaging.fileformats.emf.emfplus.records/emfplussetinterpolationmode/
---
**Inheritance:**
java.lang.Object, [com.aspose.imaging.fileformats.emf.MetaObject](../../com.aspose.imaging.fileformats.emf/metaobject), [com.aspose.imaging.fileformats.emf.emfplus.records.EmfPlusRecord](../../com.aspose.imaging.fileformats.emf.emfplus.records/emfplusrecord), [com.aspose.imaging.fileformats.emf.emfplus.records.EmfPlusPropertyRecordType](../../com.aspose.imaging.fileformats.emf.emfplus.records/emfpluspropertyrecordtype)
```
public final class EmfPlusSetInterpolationMode extends EmfPlusPropertyRecordType
```

该 EmfPlusSetInterpolationMode 记录指定图像缩放（包括拉伸和收缩）的执行方式。
## 构造函数

| 构造函数 | 描述 |
| --- | --- |
| [EmfPlusSetInterpolationMode(EmfPlusRecord source)](#EmfPlusSetInterpolationMode-com.aspose.imaging.fileformats.emf.emfplus.records.EmfPlusRecord-) | 初始化 `EmfPlusSetInterpolationMode` 类的新实例。 |
## 方法

| 方法 | 描述 |
| --- | --- |
| [getInterpolationMode()](#getInterpolationMode--) | 获取或设置插值模式值，来自 InterpolationMode 枚举（第 2.1.1.16 节）。 |
| [setInterpolationMode(byte value)](#setInterpolationMode-byte-) | 获取或设置插值模式值，来自 InterpolationMode 枚举（第 2.1.1.16 节）。 |
### EmfPlusSetInterpolationMode(EmfPlusRecord source) {#EmfPlusSetInterpolationMode-com.aspose.imaging.fileformats.emf.emfplus.records.EmfPlusRecord-}
```
public EmfPlusSetInterpolationMode(EmfPlusRecord source)
```


初始化 `EmfPlusSetInterpolationMode` 类的新实例。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| source | [EmfPlusRecord](../../com.aspose.imaging.fileformats.emf.emfplus.records/emfplusrecord) | 源。 |

### getInterpolationMode() {#getInterpolationMode--}
```
public byte getInterpolationMode()
```


获取或设置插值模式值，来自 InterpolationMode 枚举（第 2.1.1.16 节）。

值：插值模式。

**Returns:**
byte
### setInterpolationMode(byte value) {#setInterpolationMode-byte-}
```
public void setInterpolationMode(byte value)
```


获取或设置插值模式值，来自 InterpolationMode 枚举（第 2.1.1.16 节）。

值：插值模式。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | byte |  |

