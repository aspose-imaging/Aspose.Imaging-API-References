...
description: "EmfPlusSave 记录将由指定索引标识的图形状态保存到已保存图形状态的堆栈上。"
type: docs
weight: 51
...
EmfPlusSave 记录将由指定索引标识的图形状态保存在已保存的图形状态堆栈上。
## 构造函数
...
获取或设置一个 32 位无符号整数，指定与图形状态关联的级别。该级别值可由后续的 EmfPlusRestore 记录（section 2.3.7.4）操作用于检索图形状态。
