...
| [getSweepAngle()](#getSweepAngle--) | 获取扫掠角度：一个 32 位浮点值，指定要绘制的弧线的范围，以度为单位，从 StartAngle 值定义的起始点测量。 |
| [setSweepAngle(float value)](#setSweepAngle-float-) | 设置扫掠角度：一个 32 位浮点值，指定要绘制的弧线的范围，以度为单位，从 StartAngle 值定义的起始点测量。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | boolean | `true` 如果为 float；否则为 `false`。 |
