...
| [setRectangle(RectangleF value)](#setRectangle-com.aspose.imaging.RectangleF-) | 设置定义渐变起始和结束点的矩形区域。 |
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| value | [RectangleF](../../com.aspose.imaging/rectanglef) | 一个 `com.aspose.imaging.RectangleF` 结构，用于指定渐变的起始点和结束点。 |
...
**Returns:**
boolean - 如果在使用此 `LinearGradientBrushBase` 进行变换时更改了 `LinearGradientBrushBase.Angle`，则为 `true`；否则为 `false`。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | boolean | `true` 表示在使用此 `LinearGradientBrushBase` 进行变换时更改了 `LinearGradientBrushBase.Angle`；否则为 `false`。 |
...
**Returns:**
boolean - 如果为此 `LinearGradientBrushBase` 启用了伽马校正，则值为 true；否则为 false。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 值 | boolean | 如果为此 `LinearGradientBrushBase` 启用了伽马校正，则值为 true；否则为 false。 |
