---
title: "LinqWrapperException"
second_title: "Aspose.Imaging for Java API 参考文档"
description: "LinqWrapper 异常类。"
type: docs
weight: 22
url: /zh/java/com.aspose.imaging.coreexceptions/linqwrapperexception/
---
**Inheritance:**
java.lang.Object, java.lang.Throwable, java.lang.Exception, java.lang.RuntimeException, com.aspose.ms.System.Exception
```
public class LinqWrapperException extends System.Exception
```

LinqWrapper 异常类。
## 构造函数

| 构造函数 | 描述 |
| --- | --- |
| [LinqWrapperException(String message)](#LinqWrapperException-java.lang.String-) | 初始化 `LinqWrapperException` 类的新实例。 |
| [LinqWrapperException(String message, Throwable innerException)](#LinqWrapperException-java.lang.String-java.lang.Throwable-) | 初始化 `LinqWrapperException` 类的新实例。 |
### LinqWrapperException(String message) {#LinqWrapperException-java.lang.String-}
```
public LinqWrapperException(String message)
```


初始化 `LinqWrapperException` 类的新实例。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| message | java.lang.String | 描述错误的消息。 |

### LinqWrapperException(String message, Throwable innerException) {#LinqWrapperException-java.lang.String-java.lang.Throwable-}
```
public LinqWrapperException(String message, Throwable innerException)
```


初始化 `LinqWrapperException` 类的新实例。

**Parameters:**
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| message | java.lang.String | 解释异常原因的错误消息。 |
| innerException | java.lang.Throwable | 导致当前异常的异常，如果未指定内部异常，则为空引用（Visual Basic 中的 Nothing）。 |

