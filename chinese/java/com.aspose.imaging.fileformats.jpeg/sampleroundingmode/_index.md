...
将 8 位值外推以适配 n 位，其中 1 < n < 8。所有可能的 8 位值的数量为 1 << 8 = 256，范围从 0 到 255。所有可能的 n 位值的数量为 1 << n，范围从 0 到 (1 << n) - 1。对应某个 8 位值 V8 的最合理的 n 位值 Vn 等于 Vn = V8 >> (8 - n)。
...
将 8 位值截断以适配 n 位，其中 1 < n < 8。所有可能的 n 位值的数量为 1 << n，范围从 0 到 (1 << n) - 1。对应某个 8 位值 V8 的最合理的 n 位值 Vn 等于 Vn = V8 & ((1 << n) - 1)。
