...
description: "使用我们的 API 高效地操作 JPEG 栅格图像，支持多种颜色配置文件，如 RGB 和 CMYK，可自定义每像素位深分辨率，并处理 EXIF、JFIF 和 XMP 元数据容器。"
type: docs
weight: 14
...
使用我们的 API 高效地操作 JPEG 栅格图像，支持多种颜色配置文件，如 RGB 和 CMYK，可自定义每像素位深分辨率，并处理 EXIF、JFIF 和 XMP 元数据容器。享受基于方向数据的自动旋转，并可从不同的压缩级别中选择，包括无损 JPEG，以实现项目中图像质量与文件大小的最佳平衡。
## 构造函数
...
| [JpegImage(RasterImage rasterImage)](#JpegImage-com.aspose.imaging.RasterImage-) | 使用栅格图像参数，初始化 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类的新实例。 |
| [JpegImage(int width, int height)](#JpegImage-int-int-) | 使用指定的宽度和高度参数，创建 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类的新实例。 |
| [JpegImage(JpegOptions jpegOptions, int width, int height)](#JpegImage-com.aspose.imaging.imageoptions.JpegOptions-int-int-) | 使用提供的 JPEG 选项，初始化新的 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 对象。 |
## 方法

| 方法 | 描述 |
| --- | --- |
| [getFileFormat()](#getFileFormat--) | 使用此属性即可轻松获取图像的格式。 |
| [getJpegOptions()](#getJpegOptions--) | 轻松获取在创建或加载此 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 实例时使用的 JPEG 选项。 |
| [getBitsPerPixel()](#getBitsPerPixel--) | 使用此属性即可轻松获取图像的像素深度，提供颜色或灰度表示的丰富程度信息。 |
| [getComment()](#getComment--) | 使用此属性管理 JPEG 文件注释，允许您添加或获取与图像关联的描述性注释。 |
| [setComment(String value)](#setComment-java.lang.String-) | 使用此属性管理 JPEG 文件注释，允许您添加或获取与图像关联的描述性注释。 |
| [getJpegExifData()](#getJpegExifData--) | 获取 Exif 实例。 |
| [setJpegExifData(JpegExifData value)](#setJpegExifData-com.aspose.imaging.exif.JpegExifData-) | 使用此属性管理 EXIF 数据，允许您添加或获取与图像关联的元数据。 |
...
| [getHorizontalResolution()](#getHorizontalResolution--) | 此属性使您能够访问 [RasterImage](../../com.aspose.imaging/rasterimage) 的水平分辨率，单位为每英寸像素数。 |
| [setHorizontalResolution(double value)](#setHorizontalResolution-double-) | 此属性使您能够访问 [RasterImage](../../com.aspose.imaging/rasterimage) 的水平分辨率，单位为每英寸像素数。 |
| [getJfif()](#getJfif--) | 此属性允许您访问或修改与 JPEG 图像关联的 JFIF（JPEG 文件互换格式）数据。 |
| [setJfif(JFIFData value)](#setJfif-com.aspose.imaging.fileformats.jpeg.JFIFData-) | 此属性允许您访问或修改与 JPEG 图像关联的 JFIF（JPEG 文件互换格式）数据。 |
| [getRawDataFormat()](#getRawDataFormat--) | 此属性检索图像的原始数据格式，指示图像数据的结构和编码方式。 |
| [getVerticalResolution()](#getVerticalResolution--) | 此属性管理关联的 [RasterImage](../../com.aspose.imaging/rasterimage) 的垂直分辨率，单位为每英寸像素数。 |
| [setVerticalResolution(double value)](#setVerticalResolution-double-) | 此属性管理关联的 [RasterImage](../../com.aspose.imaging/rasterimage) 的垂直分辨率，单位为每英寸像素数。 |
| [getWidth()](#getWidth--) | 此属性检索图像的宽度，单位为像素。 |
...
| [getCmykColorProfile()](#getCmykColorProfile--) | CMYK 和 YCCK JPEG 图像关联的 CMYK 颜色配置文件确保精确的颜色转换和保真度。 |
| [setCmykColorProfile(StreamSource value)](#setCmykColorProfile-com.aspose.imaging.sources.StreamSource-) | CMYK 和 YCCK JPEG 图像关联的 CMYK 颜色配置文件确保精确的颜色转换和保真度。 |
...
[JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类通过使用指定的路径参数调用构造函数即可轻松实例化。此构造函数实现了 JPEG 图像的无缝创建，确保能够轻松快速地将其集成到您的项目中。
...
使用流参数，通过 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类初始化 JPEG 图像对象。此构造函数简化了 JPEG 图像的使用过程，提供了一种直接的方式，可轻松将其集成到您的项目中。
...
使用栅格图像参数初始化 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类的新实例。此构造函数提供了一种便捷方式，可直接从栅格图像创建 JPEG 图像，简化了在应用程序中使用 JPEG 图像的工作流。
...
使用指定的宽度和高度参数创建 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 类的新实例。此构造函数允许您创建具有自定义尺寸的 JPEG 图像，为在应用程序中管理图像大小提供了灵活性。
...
使用提供的 JPEG 选项初始化新的 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 对象。此构造函数使您能够定制 JPEG 图像的各种设置，如压缩级别、质量以及其他参数，从而对生成的图像格式进行精确控制。
...
通过此属性轻松获取图像的格式。它提供了有关文件格式的有价值信息，有助于在各种平台和应用程序之间实现无缝集成和兼容性检查。
...
轻松获取在创建或加载此 [JpegImage](../../com.aspose.imaging.fileformats.jpeg/jpegimage) 实例时使用的 JPEG 选项。此属性提供了有关所使用的具体设置的有价值细节，使用户能够有效地理解和复现图像处理工作流。无论是压缩级别、质量设置还是其他参数，此属性都提供了实现无缝图像操作所必需的关键洞察。
...
通过此属性轻松获取图像的像素深度，提供对颜色或灰度表现丰富程度的洞察。无论是鲜艳的照片还是单色插图，此属性都提供了关于图像视觉复杂性的关键信息。
...
使用此属性管理 JPEG 文件注释，允许您添加或检索与图像关联的描述性标注。无论是为图像添加元数据标签还是附加额外上下文，此属性都提供了在组织和分类 JPEG 文件时的灵活性。
...
使用此属性管理 EXIF 数据，允许您添加或检索与图像关联的元数据。无论是提取相机设置信息还是修改现有元数据，此属性都提供了在管理 EXIF 数据容器时的灵活性。
...
使用此属性即可轻松获取图像的高度。它提供对图像垂直尺寸的快速访问，使您能够高效地确定其大小和宽高比，而无需复杂的计算或额外的方法。
...
此属性允许您访问 [RasterImage](../../com.aspose.imaging/rasterimage) 的水平分辨率，单位为每英寸像素。通过设置或获取此值，您可以精确控制图像的分辨率，确保其满足您对质量和清晰度的特定要求。
...
此属性允许您访问或修改与 JPEG 图像关联的 JFIF（JPEG 文件交换格式）数据。JFIF 是在计算机和其他设备之间交换 JPEG 压缩图像的标准格式。通过获取或设置此属性，您可以操作 JFIF 数据，其中可能包括图像的分辨率、宽高比和缩略图等信息。
...
此属性检索图像的原始数据格式，指示图像数据的结构和编码方式。了解原始数据格式对于有效处理或操作图像数据至关重要。它提供对图像底层表示的洞察，例如是否已压缩、是否采用特定色彩空间编码或存储在特定文件格式中。访问此属性可让您获得有关图像数据结构的有价值信息，从而能够执行针对其特定格式的各种操作或优化。
...
此属性管理关联的 [RasterImage](../../com.aspose.imaging/rasterimage) 的垂直分辨率，单位为每英寸像素。调整此分辨率会影响图像在固定物理尺寸下打印或显示时的大小和质量。通过设置此属性，您可以控制图像像素在垂直方向上的密集程度，从而影响其整体清晰度和锐利度。
...
此属性检索图像的宽度，单位为像素。它提供有关图像尺寸的关键信息，使得图像数据的渲染、操作或显示能够精确进行。
...
CMYK 和 YCCK JPEG 图像的 RGB 颜色配置文件可确保准确的颜色转换和呈现。它必须与 CMYKColorProfile 配合使用，以保持颜色渲染的一致性和保真度。此配对对于需要精确颜色管理和图像再现的应用程序至关重要，确保 RGB 数据得到正确解释和显示。
...
以下示例使用自定义 ICC 配置文件加载 PNG 并将其保存为 CMYK JPEG。随后加载 CMYK JPEG 并将其保存回 PNG。RGB 到 CMYK 以及 CMYK 到 RGB 的颜色转换均使用自定义 ICC 配置文件完成。
``` java
String dir = "c:\\temp\\";
...
与 CMYK 和 YCCK JPEG 图像关联的 CMYK 颜色配置文件可确保精确的颜色转换和保真度。它与 RGBColorProfile 协同工作，以保证在各种设备和应用中的颜色呈现准确。此配对对于保持颜色渲染的一致性并实现最佳图像质量至关重要。
...
在保存过程中，RGBColorProfile 对 CMYK 和 YCCK JPEG 图像的准确颜色转换至关重要。与 CMYKColorProfile 配合使用时，它可确保颜色正确渲染，并在不同设备和应用之间保持一致性。此组合对于保留预期的颜色表现并实现高质量图像输出至关重要。
...
在保存过程中，CMYK 颜色配置文件对 CMYK 和 YCCK JPEG 图像的准确颜色转换至关重要。它与 RGBColorProfile 协同工作，以确保颜色呈现正确，并在不同设备和软件之间保持一致性和质量。此同步对于在最终保存的图像中实现准确可靠的颜色渲染至关重要。
...
检索或修改标记，指示是否忽略嵌入的颜色配置文件。通过设置此标志，用户可以指定是使用默认颜色配置文件还是嵌入的配置文件。此选项提供了对颜色管理的更大控制，便于在各种平台和应用之间进行一致性和兼容性的调整。
...
为指定的 [RasterImage](../../com.aspose.imaging/rasterimage) 设置分辨率，确保精确的缩放和打印能力。此方法使用户能够根据其特定需求（无论是数字显示还是实体复制）定制图像分辨率。通过设置分辨率，用户可以优化图像质量，并确保与各种输出设备和介质的兼容性，提升整体视觉体验和图像的可用性。
