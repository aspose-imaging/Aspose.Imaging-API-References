...
| 方法 | 描述 |
| --- | --- |
| [getDefaultPage()](#getDefaultPage--) | 使用此用户友好的属性，轻松检索图像的默认页。 |
| [isCached()](#isCached--) | 轻松确定对象的数据是否已缓存，消除读取数据的需求。 |
...
| [getFileFormat()](#getFileFormat--) | 使用此直观属性，轻松检索图像的文件格式。 |
| [getWidth()](#getWidth--) | 获取图像宽度。 |
| [getHeight()](#getHeight--) | 获取图像高度。 |
| [cacheData()](#cacheData--) | 使用此用户友好的方法，轻松缓存数据，以防止从底层来源再次加载。 |
...
通过使用流和 loadOptions 参数初始化新实例，轻松开始使用 [CdrImage](../../com.aspose.imaging.fileformats.cdr/cdrimage) 类。此方式非常适合希望以便捷方式从各种数据源加载 CDR 图像并根据需要自定义加载过程的开发者。
...
使用此用户友好的方法，轻松缓存数据，以防止从底层来源再次加载。对于希望通过预加载数据来优化性能、确保更快访问和更流畅操作的开发者而言，这一点非常理想。`DataStreamSupporter.DataStreamContainer`([DataStreamSupporter.getDataStreamContainer](../../com.aspose.imaging/datastreamsupporter\\#getDataStreamContainer)/[DataStreamSupporter.setDataStreamContainer\\_internalized(StreamContainer)](../../com.aspose.imaging/datastreamsupporter\\#setDataStreamContainer-internalized-StreamContainer-))。
