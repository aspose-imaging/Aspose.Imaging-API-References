...
| [getDigitSubstitution()](#getDigitSubstitution--) | 获取或设置一个 32 位无符号整数，指定根据区域设置或语言如何替换字符串中的数字。 |
| [setDigitSubstitution(int value)](#setDigitSubstitution-int-) | 获取或设置一个 32 位无符号整数，指定根据区域设置或语言如何替换字符串中的数字。 |
| [getFirstTabOffset()](#getFirstTabOffset--) | 获取或设置一个 32 位浮点值，指定文本行起始与第一个制表位之间的空格数。 |
| [setFirstTabOffset(float value)](#setFirstTabOffset-float-) | 获取或设置一个 32 位浮点值，指定文本行起始与第一个制表位之间的空格数。 |
...
| [getLeadingMargin()](#getLeadingMargin--) | 获取或设置一个 32 位浮点值，指定在字符串起始位置添加的空格长度。 |
| [setLeadingMargin(float value)](#setLeadingMargin-float-) | 获取或设置一个 32 位浮点值，指定在字符串起始位置添加的空格长度。 |
| [getLineAlign()](#getLineAlign--) | 获取或设置一个 32 位无符号整数，指定在布局矩形中垂直对齐字符串的方式。 |
| [setLineAlign(int value)](#setLineAlign-int-) | 获取或设置一个 32 位无符号整数，指定在布局矩形中垂直对齐字符串的方式。 |
| [getRangeCount()](#getRangeCount--) | 获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的 EmfPlusCharacterRange 对象（第 2.2.2.8 节）的数量。 |
| [setRangeCount(int value)](#setRangeCount-int-) | 获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的 EmfPlusCharacterRange 对象（第 2.2.2.8 节）的数量。 |
| [getStringAlignment()](#getStringAlignment--) | 获取或设置一个 32 位无符号整数，指定在布局矩形中水平对齐字符串的方式。 |
| [setStringAlignment(int value)](#setStringAlignment-int-) | 获取或设置一个 32 位无符号整数，指定在布局矩形中水平对齐字符串的方式。 |
...
| [getTabstopCount()](#getTabstopCount--) | 获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的制表位数量。 |
| [setTabstopCount(int value)](#setTabstopCount-int-) | 获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的制表位数量。 |
| [getTracking()](#getTracking--) | 获取或设置一个 32 位浮点值，用于指定在指定字符串中分配给每个字符的水平空间与字符的字体定义宽度的比例。 |
| [setTracking(float value)](#setTracking-float-) | 获取或设置一个 32 位浮点值，用于指定在指定字符串中分配给每个字符的水平空间与字符的字体定义宽度的比例。 |
| [getTrailingMargin()](#getTrailingMargin--) | 获取或设置一个 32 位浮点值，用于指定在字符串后保留的空格长度。 |
| [setTrailingMargin(float value)](#setTrailingMargin-float-) | 获取或设置一个 32 位浮点值，用于指定在字符串后保留的空格长度。 |
| [getTrimming()](#getTrimming--) | 获取或设置，指定如何修剪过大而无法适配布局矩形的字符串中的字符。 |
| [setTrimming(int value)](#setTrimming-int-) | 获取或设置，指定如何修剪过大而无法适配布局矩形的字符串中的字符。 |
### EmfPlusStringFormat() {#EmfPlusStringFormat--}
```
...
获取或设置一个 EmfPlusLanguageIdentifier 对象，用于指定字符串中数字使用的语言。例如，如果该字符串包含阿拉伯数字，则此字段必须包含指定阿拉伯语言的语言标识符。
...
获取或设置一个 32 位无符号整数，用于指定根据区域设置或语言在字符串中替换数字的方式。此值必须在 StringDigitSubstitution 枚举中定义（第 2.1.1.30 节）。
...
获取或设置一个 32 位浮点值，指定文本行起始与第一个制表位之间的空格数。
...
获取或设置一个 32 位有符号整数，用于指定在遇到键盘快捷键前缀（即 & 符号）时对字符串执行的处理类型。基本上，此字段指定是否显示与文本相关的键盘快捷键前缀。该值必须在 HotkeyPrefix 枚举中定义（第 2.1.1.14 节）。
...
获取或设置一个 32 位浮点值，用于指定在字符串起始位置添加的空格长度。默认值为 1/6 英寸；对于排版字体，默认值为 0。
...
获取或设置一个 32 位无符号整数，用于指定字符串在布局矩形中的垂直对齐方式。此值必须在 StringAlignment 枚举中定义。
...
获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的 EmfPlusCharacterRange 对象（第 2.2.2.8 节）的数量。
...
获取或设置一个 32 位无符号整数，用于指定字符串在布局矩形中的水平对齐方式。此值必须在 StringAlignment 枚举中定义（第 2.1.1.29 节）。
...
获取或设置一个 32 位无符号整数，用于指定文本布局的格式化、裁剪和字体处理选项。此值必须由 StringFormat 标志组成（第 2.1.2.8 节）。
...
获取或设置一个 32 位有符号整数，指定 StringFormatData 字段中定义的制表位数量。
...
获取或设置一个 32 位浮点值，用于指定在指定字符串中分配给每个字符的水平空间与字符的字体定义宽度的比例。该属性的较大值表示字符之间有充足的间距；小于 1 的值可能导致字符重叠。默认值为 1.03；对于排版字体，默认值为 1.00。
...
获取或设置一个 32 位浮点值，用于指定在字符串后保留的空格长度。默认值为 1/6 英寸；对于排版字体，默认值为 0。
...
获取或设置，指定如何修剪过大而无法适配布局矩形的字符串中的字符。此值必须在 StringTrimming 枚举中定义（第 2.1.1.31 节）。
