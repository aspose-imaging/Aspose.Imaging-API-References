...
| 方法 | 描述 |
| --- | --- |
| [getImageData()](#getImageData--) | 获取或设置图像数据的可变长度数据，该数据定义了 Type 字段中指定的图像数据。 |
| [setImageData(EmfPlusBaseImageData value)](#setImageData-com.aspose.imaging.fileformats.emf.emfplus.objects.EmfPlusBaseImageData-) | 获取或设置图像数据的可变长度数据，该数据定义了 Type 字段中指定的图像数据。 |
| [getType()](#getType--) | 获取或设置图像类型，一个 32 位无符号整数，指定 ImageData 字段中数据的类型。 |
| [setType(int value)](#setType-int-) | 获取或设置图像类型，一个 32 位无符号整数，指定 ImageData 字段中数据的类型。 |
### EmfPlusImage() {#EmfPlusImage--}
```
public EmfPlusImage()
```
...
获取或设置图像数据的可变长度数据，该数据定义了 Type 字段中指定的图像数据。数据的内容和格式可能因每种图像类型而异。
...
获取或设置图像类型的 32 位无符号整数，指定 ImageData 字段中的数据类型。此值必须在 ImageDataType 枚举中定义（第 2.1.1.15 节）。
