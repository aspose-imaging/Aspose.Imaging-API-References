...
获取或设置一个 32 位有符号整数，指定亮度级别。该值必须在 -255 到 255 的范围内，其效果如下： -255 \\u2264 value < 0 当值减小时，图像的亮度应降低。 0 值为 0 表示亮度不得改变。 0 < value \\u2264 255 当值增大时，图像的亮度应增加。
...
获取或设置一个 32 位有符号整数，指定对比度级别。该值必须在 -100 到 100 的范围内，其效果如下： -100 \\u2264 value < 0 当值减小时，图像的对比度应降低。 0 值为 0 表示对比度不得改变。 0 < value \\u2264 100 当值增大时，图像的对比度应增加。
