...
description: "该 EmfPlusRegion 对象指定定义非直线形状的线段和曲线段"
type: docs
weight: 68
...
该 EmfPlusRegion 对象指定定义非直线形状的线段和曲线段
## 构造函数
...
| 方法 | 描述 |
| --- | --- |
| [getRegionNode()](#getRegionNode--) | 获取或设置 RegionNodeCount+1 EmfPlusRegionNode 对象的数组（第 2.2.2.40 节）。 |
| [setRegionNode(EmfPlusRegionNode[] value)](#setRegionNode-com.aspose.imaging.fileformats.emf.emfplus.objects.EmfPlusRegionNode---) | 获取或设置 RegionNodeCount+1 EmfPlusRegionNode 对象的数组（第 2.2.2.40 节）。 |
### EmfPlusRegion() {#EmfPlusRegion--}
```
...
获取或设置 RegionNodeCount+1 EmfPlusRegionNode 对象的数组（第 2.2.2.40 节）。区域被指定为区域节点的二叉树，每个节点必须是终端节点或指定一个或两个子节点。RegionNode 必须至少包含一个元素。
