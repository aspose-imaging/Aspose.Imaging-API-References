...
| 方法 | 描述 |
| --- | --- |
| [getBrushDataFlags()](#getBrushDataFlags--) | 获取或设置画笔数据标志。 |
| [setBrushDataFlags(int value)](#setBrushDataFlags-int-) | 获取或设置画笔数据标志。 |
...
值：BrushDataFlags（4 字节）：一个 32 位无符号整数，指定 OptionalData 字段中的数据。此值必须由 `EmfPlusBrushDataFlags`（第 2.1.2.1 节）组成。
...
值：一个 EmfPlusRectF 对象（第 2.2.2.39 节），指定渐变线的起始点和结束点。矩形的左上角为起始点，右下角为结束点。
...
值：一个来自 WrapMode 枚举的 32 位有符号整数（第 2.1.1.34 节），指定是否绘制画笔边界之外的区域。在绘制边界之外时，包装模式指定颜色渐变的重复方式。
