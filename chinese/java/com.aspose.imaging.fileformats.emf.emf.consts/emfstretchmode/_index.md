...
description: "StretchMode 枚举用于指定在拉伸或压缩位图时如何添加或移除颜色数据。"
type: docs
weight: 43
...
StretchMode 枚举用于指定在拉伸或压缩位图时如何添加或移除颜色数据。
## 字段

| 字段 | 描述 |
| --- | --- |
| [STRETCH_ANDSCANS](#STRETCH-ANDSCANS) | 使用被消除像素和现有像素的颜色值执行布尔 AND 运算。 |
| [STRETCH_ORSCANS](#STRETCH-ORSCANS) | 使用被消除像素和现有像素的颜色值执行布尔 OR 运算。 |
...
使用被消除像素和现有像素的颜色值执行布尔 AND 操作。 如果位图是单色位图，此模式会以牺牲白色像素为代价保留黑色像素
...
使用被消除像素和现有像素的颜色值执行布尔 OR 操作。 如果位图是单色位图，此模式会以牺牲黑色像素为代价保留白色像素。
