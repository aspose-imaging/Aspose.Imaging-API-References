...
| [MWT_LEFTMULTIPLY](#MWT-LEFTMULTIPLY) | 对当前变换进行乘法。 |
| [MWT_RIGHTMULTIPLY](#MWT-RIGHTMULTIPLY) | 对当前变换进行乘法。 |
| [MWT_SET](#MWT-SET) | 执行 EMR\\_SETWORLDTRANSFORM 记录的功能（第 2.3.12.2 节）。 |
...
对当前变换进行乘法。在此模式下，指定的变换数据为左乘数，而播放设备上下文中当前定义的变换为右乘数。
...
对当前变换进行乘法。在此模式下，指定的变换数据为右乘数，而播放设备上下文中当前定义的变换为左乘数。
...
执行 EMR\\_SETWORLDTRANSFORM 记录的功能（第 2.3.12.2 节）。
