...
description: "PenStyle 枚举定义了可在图形操作中使用的笔的属性。"
type: docs
weight: 34
...
PenStyle 枚举定义了可用于图形操作的画笔属性。画笔样式是画笔类型、线条样式、线端帽和线连接的组合。
## 字段

| 字段 | 描述 |
| --- | --- |
| [PS_COSMETIC](#PS-COSMETIC) | 一种画笔类型，指定宽度为一个逻辑单元且样式为纯色的线条 |
| [PS_ENDCAP_ROUND](#PS-ENDCAP-ROUND) | 一种线端帽，指定圆形端点。 |
| [PS_JOIN_ROUND](#PS-JOIN-ROUND) | 一种线连接，指定圆形连接。 |
| [PS_SOLID](#PS-SOLID) | 一种线条样式，为纯色。 |
| [PS_DASH](#PS-DASH) | 一种线条样式，为虚线。 |
| [PS_DOT](#PS-DOT) | 一种线条样式，为点线。 |
| [PS_DASHDOT](#PS-DASHDOT) | 一种线条样式，由交替的短划线和点组成。 |
| [PS_DASHDOTDOT](#PS-DASHDOTDOT) | 一种由破折号和双点组成的线型。 |
| [PS_NULL](#PS-NULL) | 一种不可见的线型。 |
| [PS_INSIDEFRAME](#PS-INSIDEFRAME) | 一种纯色线型。 |
| [PS_USERSTYLE](#PS-USERSTYLE) | 一种由样式数组定义的线型，该数组指定线段中破折号和间隙的长度 |
| [PS_ALTERNATE](#PS-ALTERNATE) | 一种每隔一个像素设置的线型。 |
| [PS_ENDCAP_SQUARE](#PS-ENDCAP-SQUARE) | 一种指定方形端点的线帽。 |
| [PS_ENDCAP_FLAT](#PS-ENDCAP-FLAT) | 一种指定平直端点的线帽。 |
| [PS_JOIN_BEVEL](#PS-JOIN-BEVEL) | 一种指定斜角连接的线段连接方式。 |
| [PS_JOIN_MITER](#PS-JOIN-MITER) | 一种在连接长度在播放设备上下文中设置的当前斜接长度限制范围内时，指定斜接连接的线段连接方式。 |
| [PS_GEOMETRIC](#PS-GEOMETRIC) | 一种笔类型，指定以逻辑单位测量宽度的线条，并且其样式可以包含画刷的任何属性。 |
...
一种纯色线型。当在接受边界矩形的绘图记录中指定此样式时，图形的尺寸会被缩小，以便完全适应边界矩形，并考虑笔的宽度。
...
一种由样式数组定义的线型，该数组指定线段中破折号和间隙的长度
...
一种每隔一个像素设置的线型。此样式仅适用于 PS\_COSMETIC 笔类型。
...
一种在连接长度在播放设备上下文中设置的当前斜接长度限制范围内时，指定斜接连接的线段连接方式。如果连接长度超过斜接限制，则指定斜角连接。
...
一种笔类型，指定以逻辑单位测量宽度的线条，并且其样式可以包含画刷的任何属性。
