...
description: "EmrComment 枚举定义了公共注释记录可以包含的数据类型，如第 2.3.3.4 节所述。"
type: docs
weight: 18
...
EmrComment 枚举定义了公共注释记录可以包含的数据类型，如第 2.3.3.4 节所述。
## 字段
...
| [EMR_COMMENT_MULTIFORMATS](#EMR-COMMENT-MULTIFORMATS) | 此注释记录允许在元文件中包含多个图像定义。 |
...
此注释记录包含 WMF 中图像的规范。有关更多信息，请参阅 [MS-WMF]。
...
此注释记录标识一组绘图记录的结束。对于每个 EMR\_COMMENT\_BEGINGROUP 记录，元文件中必须包含相应的 EMR\_COMMENT\_ENDGROUP 记录，并且它们可以嵌套。
...
此注释记录允许在元文件中包含多个图像定义。例如，使用此注释，应用程序可以同时包含封装的 PostScript 文本以及图像的 EMF 定义。
