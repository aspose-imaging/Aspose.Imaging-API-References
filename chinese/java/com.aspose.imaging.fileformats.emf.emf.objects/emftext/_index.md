...
| 方法 | 描述 |
| --- | --- |
| [getReference()](#getReference--) | 获取或设置 WMF PointL 对象（[MS-WMF] 第 2.2.2.15 节），该对象指定用于定位字符串的参考点坐标。 |
| [setReference(Point value)](#setReference-com.aspose.imaging.Point-) | 获取或设置 WMF PointL 对象（[MS-WMF] 第 2.2.2.15 节），该对象指定用于定位字符串的参考点坐标。 |
| [getChars()](#getChars--) | 获取或设置指定字符串中字符数的 32 位无符号整数。 |
| [setChars(int value)](#setChars-int-) | 获取或设置指定字符串中字符数的 32 位无符号整数。 |
| [getOptions()](#getOptions--) | 获取或设置指定如何使用 Rectangle 字段中矩形的 32 位无符号整数。 |
| [setOptions(int value)](#setOptions-int-) | 获取或设置指定如何使用 Rectangle 字段中矩形的 32 位无符号整数。 |
| [getRectangle()](#getRectangle--) | 获取或设置可选的 WMF RectL 对象（[MS-WMF] 第 2.2.2.19 节），该对象以逻辑单位定义裁剪和/或不透明矩形。 |
| [setRectangle(Rectangle value)](#setRectangle-com.aspose.imaging.Rectangle-) | 获取或设置可选的 WMF RectL 对象（[MS-WMF] 第 2.2.2.19 节），该对象以逻辑单位定义裁剪和/或不透明矩形。 |
...
获取或设置 WMF PointL 对象（[MS-WMF] 第 2.2.2.15 节），该对象指定用于定位字符串的参考点坐标。参考点由最后一个 EMR\_SETTEXTALIGN 记录（第 2.3.11.25 节）定义。如果未设置此记录，默认对齐方式为 TA\_LEFT,TA\_TOP。
...
获取可选的字形索引缓冲区。如果 options 含有 ETO\_GLYPH\_INDEX 标志，则输出文本字符串中字符的代码实际上是 TrueType 字体（2.1.11 ExtTextOutOptions 枚举）中字符字形的索引。字形索引是特定于字体的，因此要在回放时显示正确的字符，使用的字体必须与生成索引时使用的字体完全相同。
...
设置可选的字形索引缓冲区。如果 options 含有 ETO\_GLYPH\_INDEX 标志，则输出文本字符串中字符的代码实际上是 TrueType 字体（2.1.11 ExtTextOutOptions 枚举）中字符字形的索引。字形索引是特定于字体的，因此要在回放时显示正确的字符，使用的字体必须与生成索引时使用的字体完全相同。
