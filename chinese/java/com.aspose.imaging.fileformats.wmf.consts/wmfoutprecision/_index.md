...
description: "OutPrecision 枚举定义了输出精度的值，这是字体映射器匹配特定字体参数（包括高度、宽度、字符方向、倾斜、间距和字体类型）的要求。"
type: docs
weight: 27
...
OutPrecision 枚举定义输出精度的值，这是字体映射器匹配特定字体参数（包括高度、宽度、字符方向、倾斜角、字距和字体类型）的要求。
## 字段

| 字段 | 描述 |
| --- | --- |
| [Default](#Default) | 指定默认行为的值。 |
| [String](#String) | 在枚举栅格化字体时返回的值。 |
| [Stroke](#Stroke) | 在枚举 TrueType 及其他轮廓字体和矢量字体时返回的值。 |
| [Tt](#Tt) | 指定在系统中存在同名多个字体时选择 TrueType 字体的值。 |
| [Device](#Device) | 指定在系统中存在同名多个字体时选择设备字体的值。 |
| [Raster](#Raster) | 指定在系统中存在同名多个字体时选择栅格化字体的值。 |
| [TtOnly](#TtOnly) | 指定仅需要 TrueType 字体的值。 |
| [Outline](#Outline) | 指定需要 TrueType 和其他轮廓字体的值。 |
| [ScreenOutline](#ScreenOutline) | 指定对 TrueType 和其他轮廓字体的偏好的值。 |
| [PsOnly](#PsOnly) | 指定仅限 PostScript 字体的要求的值。 |
### Default {#Default}
```
...
指定仅限 TrueType 字体的要求的值。如果系统中未安装 TrueType 字体，则指定默认行为。
...
指定仅限 PostScript 字体的要求的值。如果系统中未安装 PostScript 字体，则指定默认行为。
