...
| 字段 | 描述 |
| --- | --- |
| [BlackOnWhite](#BlackOnWhite) | 通过使用被消除像素和现有像素的颜色值执行布尔 AND 操作。 |
| [WhiteOnBlack](#WhiteOnBlack) | 通过使用被消除像素和现有像素的颜色值执行布尔 OR 操作。 |
...
通过使用被消除像素和现有像素的颜色值执行布尔 AND 操作。如果位图是单色位图，此模式会保留黑色像素而牺牲白色像素。
...
通过使用被消除像素和现有像素的颜色值执行布尔 OR 操作。如果位图是单色位图，此模式会保留白色像素而牺牲黑色像素。
