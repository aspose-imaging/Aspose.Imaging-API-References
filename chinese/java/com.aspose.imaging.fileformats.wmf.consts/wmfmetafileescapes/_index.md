...
description: "MetafileEscapes 枚举指定打印机驱动程序功能，这些功能可能无法直接通过 RecordType 枚举第 2.1.1.1 节中定义的 WMF 记录访问。"
type: docs
weight: 24
...
| [Nextband](#Nextband) | 通知打印机驱动程序，应用程序已完成对波段的写入。 |
| [Setcolortable](#Setcolortable) | 设置颜色表值。 |
| [Getcolortable](#Getcolortable) | 获取颜色表值。 |
| [Flushout](#Flushout) | 导致所有挂起的输出被刷新到输出设备。 |
| [Draftmode](#Draftmode) | 指示打印机驱动程序应仅打印文本，而不打印图形。 |
| [Queryescsupport](#Queryescsupport) | 查询打印机驱动程序，以确定其驱动的输出设备是否支持特定的转义函数。 |
| [Setabortproc](#Setabortproc) | 设置应用程序定义的函数，以便在打印过程中取消打印作业。 |
| [Startdoc](#Startdoc) | 通知打印机驱动程序，新打印作业即将开始。 |
| [Enddoc](#Enddoc) | 通知打印机驱动程序，当前打印作业即将结束。 |
| [Getphyspagesize](#Getphyspagesize) | 检索当前在输出设备上选择的物理页面尺寸。 |
| [Getprintingoffset](#Getprintingoffset) | 检索实际打印或绘制开始的物理页面左上角的偏移量。 |
...
| [Setpapersource](#Setpapersource) | 设置输出表单的来源，例如打印机上的特定纸盒或纸槽。 |
| [Passthrough](#Passthrough) | 此记录传递任意数据。 |
| [Gettechnology](#Gettechnology) | 获取关于设备支持的图形技术的信息。 |
...
| [Bandinfo](#Bandinfo) | 检索或指定有关设备条纹的设置，例如条带数量。 |
| [Drawpatternrect](#Drawpatternrect) | 使用定义的图案绘制矩形。 |
| [Getvectorpensize](#Getvectorpensize) | 检索当前在设备上定义的物理笔尺寸。 |
| [Getvectorbrushsize](#Getvectorbrushsize) | 检索当前在设备上定义的物理刷子尺寸。 |
...
| [Epsprinting](#Epsprinting) | 指示封装的 PostScript（EPS）段的开始和结束。 |
...
| [Exttextout](#Exttextout) | 使用当前选定的字体、背景颜色和文字颜色绘制文本。 |
| [Getfacename](#Getfacename) | 获取当前在设备上配置的字体名称。 |
| [Downloadface](#Downloadface) | 在设备上设置字体名称。 |
| [MetafileDriver](#MetafileDriver) | 查询打印机驱动程序是否支持在输出设备上使用元文件。 |
| [Querydibsupport](#Querydibsupport) | 查询打印机驱动程序是否支持在输出设备上使用 DIB。 |
| [BeginPath](#BeginPath) | 打开路径。 |
| [ClipToPath](#ClipToPath) | 定义由路径限定的裁剪区域。 |
...
| [PostscriptPassthrough](#PostscriptPassthrough) | 将任意数据直接发送到打印机驱动程序，预计仅在 PostScript 模式下处理这些数据。 |
...
| [MxdcEscape](#MxdcEscape) | 使应用程序能够以 XML 纸张规范 (XPS) 格式将文档写入文件或打印机。 |
...
定义由路径限定的裁剪区域。输入必须是定义要执行操作的 16 位量。
...
将任意数据直接发送到打印机驱动程序，预计仅在 PostScript 模式下处理这些数据。 [PostscriptIdentify](../../com.aspose.imaging.fileformats.wmf.consts/wmfmetafileescapes\#PostscriptIdentify)。
...
使应用程序能够以 XML 纸张规范 (XPS) 格式将文档写入文件或打印机。
