...
PitchFont 枚举定义用于指定字体特性的值。这些值用于指示字体中的字符是固定宽度还是可变宽度（或字距）。
## 字段

| 字段 | 描述 |
| --- | --- |
| [DEFAULT_PITCH](#DEFAULT-PITCH) | 默认字距，取决于实现。 |
| [FIXED_PITCH](#FIXED-PITCH) | 固定字距，意味着字体中的所有字符在字符串输出时占用相同的宽度。 |
| [VARIABLE_PITCH](#VARIABLE-PITCH) | 可变字距，意味着字体中的字符在字符串输出时占用的宽度与字形的实际宽度成比例。 |
...
固定字距，意味着字体中的所有字符在字符串输出时占用相同的宽度。
...
可变字距，意味着字体中的字符在字符串输出时占用的宽度与字形的实际宽度成比例。例如，\"i\" 和空格字符的宽度通常远小于 \"W\" 或 \"O\" 字符的宽度。
