---
title: WebPImage.FileFormat
second_title: Aspose.Imaging for .NET API Reference
description: WebPImage property. Access the file format value associated with the image providing information about the format in which the image is stored. Utilize this property to determine the file format of the image facilitating compatibility checks and formatspecific processing within your application
type: docs
weight: 20
url: /net/aspose.imaging.fileformats.webp/webpimage/fileformat/
---
## WebPImage.FileFormat property

Access the file format value associated with the image, providing information about the format in which the image is stored. Utilize this property to determine the file format of the image, facilitating compatibility checks and format-specific processing within your application.

```csharp
public override FileFormat FileFormat { get; }
```

### See Also

* enum [FileFormat](../../../aspose.imaging/fileformat/)
* class [WebPImage](../)
* namespace [Aspose.Imaging.FileFormats.Webp](../../webpimage/)
* assembly [Aspose.Imaging](../../../)


