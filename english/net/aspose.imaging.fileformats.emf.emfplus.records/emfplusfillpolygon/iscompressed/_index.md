---
title: EmfPlusFillPolygon.IsCompressed
second_title: Aspose.Imaging for .NET API Reference
description: EmfPlusFillPolygon property. Gets or sets a value indicating whether this instance is compressed. If set PointData specifies absolute locations in the coordinate space with 16bit integer coordinates. If clear PointData specifies absolute locations in the coordinate space with 32bit floatingpoint coordinates
type: docs
weight: 40
url: /net/aspose.imaging.fileformats.emf.emfplus.records/emfplusfillpolygon/iscompressed/
---
## EmfPlusFillPolygon.IsCompressed property

Gets or sets a value indicating whether this instance is compressed. If set, PointData specifies absolute locations in the coordinate space with 16-bit integer coordinates. If clear, PointData specifies absolute locations in the coordinate space with 32-bit floating-point coordinates

```csharp
public bool IsCompressed { get; set; }
```

### Property Value

`true` if this instance is compressed; otherwise, `false`.

### See Also

* class [EmfPlusFillPolygon](../)
* namespace [Aspose.Imaging.FileFormats.Emf.EmfPlus.Records](../../emfplusfillpolygon/)
* assembly [Aspose.Imaging](../../../)


