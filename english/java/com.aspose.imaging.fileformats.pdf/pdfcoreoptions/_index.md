...
| Method | Description |
| --- | --- |
| [getHeadingsOutlineLevels()](#getHeadingsOutlineLevels--) | Specifies how many levels of outline items to include in the document outline. 0 - no outline, 1 - one outline level and so on. |
| [setHeadingsOutlineLevels(int value)](#setHeadingsOutlineLevels-int-) | Specifies how many levels of outline items to include in the document outline. 0 - no outline, 1 - one outline level and so on. |
| [getExpandedOutlineLevels()](#getExpandedOutlineLevels--) | Specifies how many levels in the document outline to show expanded when the PDF file is viewed. 0 - the document outline is not expanded. 1 - first level items in the document are expanded and so on. |
| [setExpandedOutlineLevels(int value)](#setExpandedOutlineLevels-int-) | Specifies how many levels in the document outline to show expanded when the PDF file is viewed. 0 - the document outline is not expanded. 1 - first level items in the document are expanded and so on. |
| [getBookmarksOutlineLevel()](#getBookmarksOutlineLevel--) | Specifies at which level in the document outline to display bookmark objects. 0 - not displayed. 1 at first level and so on. |
| [setBookmarksOutlineLevel(int value)](#setBookmarksOutlineLevel-int-) | Specifies at which level in the document outline to display bookmark objects. 0 - not displayed. 1 at first level and so on. |
