...
description: Инициализирует новый экземпляр классаBmpImageaspose.imaging.fileformats.bmp/bmpimage.
type: docs
weight: 10
...
Инициализирует новый экземпляр класса[`BmpImage`](../../bmpimage).
...
string dir = "c:\\temp\\";
 ...
 // При необходимости исходные пиксели будут преобразованы в формат 32 бит на пикс.
...
| --- | --- | --- |
| path | String | Путь для загрузки изображения и инициализации данных пикселей и палитры. |
| bitsPerPixel | UInt16 | Количество бит на пиксель. |
...
В примере показано, как загрузить BmpImage из файла с заданной разрядностью и разрешением.
...
string dir = "c:\\temp\\";
 ...
 // Загрузить изображение BMP из файла.
 // При необходимости исходные пиксели будут преобразованы в формат 24 бит на пикс.
...
В примере показано, как загрузить BmpImage из файлового потока с заданной разрядностью и разрешением.
...
// Создаем новое изображение в формате PNG.
...
В примере показано, как загрузить BmpImage из другого экземпляра RasterImage с указанной битовой глубиной и сжатием.
...
В примере показано, как создать BmpImage заданного размера.
...
В примере показано, как создать BmpImage с использованием различных опций.
