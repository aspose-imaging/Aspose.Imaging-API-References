...
| Имя | Описание |
| --- | --- |
| [EmfPlusSetTextRenderingHint](emfplussettextrenderinghint)(EmfPlusRecord) | Инициализирует новый экземпляр[`EmfPlusSetTextRenderingHint`](../emfplussettextrenderinghint) класс. |
...
| [TextRenderingHint](../../aspose.imaging.fileformats.emf.emfplus.records/emfplussettextrenderinghint/textrenderinghint) { get; set; } | Получает или задает значение подсказки рендеринга текста из перечисления TextRenderingHint (раздел 2.1.1.32), которое определяет качество, которое используется при последующем рендеринге текста. |
