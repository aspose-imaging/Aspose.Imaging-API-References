...
description: Типы записей состояния определяют операции над состоянием контекста устройства воспроизведения.
type: docs
weight: 6430
url: /ru/net/aspose.imaging.fileformats.emf.emfplus.records/emfplusstaterecordtype/
---
## EmfPlusStateRecordType class

Типы записей состояния определяют операции над состоянием контекста устройства воспроизведения.
...
Каждый контейнер графического состояния ДОЛЖЕН быть добавлен в массив сохраненных графических контейнеров. Контейнер состояния graphics не записывается в метафайл EMF+, поэтому его формат может определяться реализацией .
