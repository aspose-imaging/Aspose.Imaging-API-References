---
title: GetFittingRectangle
second_title: Справочник по Aspose.Imaging for .NET API
description: Получает прямоугольник соответствующий текущему изображению.
type: docs
weight: 280
url: /ru/net/aspose.imaging/image/getfittingrectangle/
---
## GetFittingRectangle(Rectangle, int, int) {#getfittingrectangle}

Получает прямоугольник, соответствующий текущему изображению.

```csharp
public static Rectangle GetFittingRectangle(Rectangle rectangle, int width, int height)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| rectangle | Rectangle | Прямоугольник, для которого нужно получить подходящий прямоугольник. |
| width | Int32 | Ширина объекта. |
| height | Int32 | Высота объекта. |

### Возвращаемое значение

Подходящий прямоугольник или исключение, если подходящего прямоугольника не найдено.

### Смотрите также

* struct [Rectangle](../../rectangle)
* class [Image](../../image)
* пространство имен [Aspose.Imaging](../../image)
* сборка [Aspose.Imaging](../../../)

---

## GetFittingRectangle(Rectangle, int[], int, int) {#getfittingrectangle_1}

Получает прямоугольник, соответствующий текущему изображению.

```csharp
public static Rectangle GetFittingRectangle(Rectangle rectangle, int[] pixels, int width, 
    int height)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| rectangle | Rectangle | Прямоугольник, для которого нужно получить подходящий прямоугольник. |
| pixels | Int32[] | 32-битные пиксели ARGB. |
| width | Int32 | Ширина объекта. |
| height | Int32 | Высота объекта. |

### Возвращаемое значение

Подходящий прямоугольник или исключение, если подходящего прямоугольника не найдено.

### Смотрите также

* struct [Rectangle](../../rectangle)
* class [Image](../../image)
* пространство имен [Aspose.Imaging](../../image)
* сборка [Aspose.Imaging](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Imaging.dll -->
