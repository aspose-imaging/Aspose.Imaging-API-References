---
title: "Класс MetafileRecorderGraphics2D"
type: docs
weight: 20
url: /ru/python-net/aspose.imaging.fileformats.emf.graphics/metafilerecordergraphics2d/
---
...
| [exclude_clip(rect)](#exclude_clip_rect_21) | Обновляет область отсечения этого Graphics, исключая область, указанную структурой Rectangle. |
| [exclude_clip(region)](#exclude_clip_region_22) | Обновляет область отсечения этого Graphics, исключая область, указанную объектом Region. |
| [exclude_clip_rect(rect)](#exclude_clip_rect_rect_23) | Обновляет область отсечения этого Graphics, исключая область, указанную структурой Rectangle. |
| [exclude_clip_rgn(region)](#exclude_clip_rgn_region_24) | Обновляет область отсечения этого Graphics, исключая область, указанную объектом Region. |
| [fill_ellipse(brush, rect)](#fill_ellipse_brush_rect_25) | Заполняет эллипс. |
| [fill_path(pen, brush, path)](#fill_path_pen_brush_path_26) | Заполняет путь. |
| [fill_pie(brush, rect, start_angle, sweep_angle)](#fill_pie_brush_rect_start_angle_sweep_angle_27) | Заполняет сектор. |
| [fill_polygon(brush, points)](#fill_polygon_brush_points_28) | Заполняет многоугольник. |
| [fill_polygon(brush, points, fill_mode)](#fill_polygon_brush_points_fill_mode_29) | Заполняет многоугольник. |
| [fill_rectangle(brush, rectangle)](#fill_rectangle_brush_rectangle_30) | Заполняет прямоугольник. |
| [get_transform()](#get_transform__31) | Получает мировое преобразование. |
| [intersect_clip(rect)](#intersect_clip_rect_32) | Обновляет область отсечения этого Graphics до пересечения текущей области отсечения и указанной структуры Rectangle. |
| [intersect_clip(region)](#intersect_clip_region_33) | Обновляет область отсечения этого Graphics до пересечения текущей области отсечения и указанного объекта Region. |
| [intersect_clip_rect_f(rect)](#intersect_clip_rect_f_rect_34) | Обновляет область отсечения этого Graphics до пересечения текущей области отсечения и указанной структуры Rectangle. |
| [intersect_clip_rgn(region)](#intersect_clip_rgn_region_35) | Обновляет область отсечения этого Graphics до пересечения текущей области отсечения и указанного объекта Region. |
| [multiply_transform(matrix)](#multiply_transform_matrix_36) | Умножает мировое преобразование этого Graphics на указанный Matrix. |
| [multiply_transform(matrix, order)](#multiply_transform_matrix_order_37) | Умножает мировое преобразование этого Graphics на указанный Matrix в указанном порядке. |
| reset_clip() | Сбрасывает обрезку. |
| [rotate_transform(angle)](#rotate_transform_angle_38) | Применяет указанное вращение к матрице преобразования этого Graphics. |
| [rotate_transform(angle, center, order)](#rotate_transform_angle_center_order_39) | Применяет указанное вращение к матрице преобразования этого Graphics в указанном порядке. |
| [scale_transform(sx, sy)](#scale_transform_sx_sy_40) | Применяет указанную операцию масштабирования к матрице преобразования этого Graphics, предваряя её матрицей преобразования объекта. |
| [scale_transform(sx, sy, order)](#scale_transform_sx_sy_order_41) | Применяет указанную операцию масштабирования к матрице преобразования этого Graphics в указанном порядке. |
| [set_transform(transform)](#set_transform_transform_42) | Устанавливает преобразование. |
| [translate_transform(x, y)](#translate_transform_x_y_43) | Изменяет начало координат, предваряя указанное перемещение матрицей преобразования этого Graphics. |
| [translate_transform(x, y, order)](#translate_transform_x_y_order_44) | Изменяет начало координат, применяя указанное перемещение к матрице преобразования этого Graphics в указанном порядке. |
