...
type: docs
weight: 7060
url: /ru/python-net/aspose.imaging/rasterimage/
---
...
| [premultiply_components](#premultiply_components2) | bool | r/w | Получает или задает значение, указывающее, должны ли компоненты изображения быть предварительно умножены. |
...
| [raw_data_format](#raw_data_format3) | [PixelDataFormat](/imaging/python-net/aspose.imaging/pixeldataformat/) | r | Получает формат необработанных данных. |
...
| auto_brightness_contrast() | Автоматическая адаптивная нормализация яркости и контрастности для всего изображения. |
...
| [crop(rectangle)](#crop_rectangle_31) | Обрезает указанный прямоугольник. |
...
| [resize(new_width, new_height, settings)](#resize_new_width_new_height_settings_82) | Изменяет размер изображения с расширенными параметрами. |
...
| [save_pixels(rectangle, pixels)](#save_pixels_rectangle_pixels_106) | Сохраняет пиксели. |
...
| [try_set_metadata(metadata)](#try_set_metadata_metadata_117) | Пытается установить экземпляр _metadata_, если данный экземпляр [Image](/imaging/python-net/aspose.imaging/image/) поддерживает и реализует тип [IImageMetadataFormat](/imaging/python-net/aspose.imaging.metadata/iimagemetadataformat/). |
...
# Загрузите изображение из PNG‑файла.
with Image.load(path_join(directory, "sample.png")) as image:
	# Кешировать все данные пикселей, чтобы не происходила дополнительная загрузка данных из базового потока данных
	image.cache_daata()
...
with Image.load(path_join(directory, "sample.png")) as image:
	...
	# Чтение всех пикселей не так быстро, как при кешировании
	rasterImage = as_of(image, RasterImage)
	for y in range(image.height):
		for x in range(image.width):
			color = rasterImage.get_argb_32_pixel(x, y)
...
with Image.create(create_options, image_width, image_height) as image:
	...
	# Пиксели хранятся в оригинальном изображении в непрепомноженной форме.
	# Необходимо явно указать соответствующую опцию, чтобы получить препомноженные компоненты цвета.
	# Препомноженные компоненты цвета рассчитываются по формулам:
...
# Вывод может выглядеть так:
# ImageFile=c:\temp\sample.bmp, FileFormat=Rgb24Bpp, использованные каналы: 8,8,8, HasAlpha=False
# ImageFile=c:\temp\alpha.png, FileFormat=RGBA32Bpp, использованные каналы: 8,8,8,8, HasAlpha=True
...
with Image.load(join_path(directory, "sample.png")) as image:
	...
	# Выполнить дизеринг Флойда с использованием 1‑битовой цветовой палитры, содержащей только 2 цвета — чёрный и белый.
	# Чем больше указано бит, тем выше качество и тем больше размер выходного изображения.
	# Обратите внимание, что в данный момент поддерживаются только 1‑битовые, 4‑битовые и 8‑битовые палитры.
...
with Image.load("sample.png") as image:
	...
	# Чтобы получить значения отдельных компонентов цвета, сдвиньте значение цвета на соответствующее количество бит.
	alpha = (color >> 24) & 0xff
	red = (color >> 16) & 0xff
	green = (color >> 8) & 0xff
	blue = (color >> 0) & 0xff
...
with Image.load("sample.png") as image:
	...
	# Другой способ — передать экземпляр aspose.imaging.Color напрямую
...
with Image.load("alpha.png") as image:
	...
	# Загрузить пиксели для всего изображения. Любую прямоугольную часть изображения можно указать в качестве параметра метода aspose.imaging.RasterImage.load_argb_32_pixels(rectangle).
...
with Image.load("16rgba.png") as image:
	rasterImage = as_of(image, RasterImage)
	# Загрузить пиксели для всего изображения. Любую прямоугольную часть изображения можно указать в качестве параметра метода aspose.imaging.RasterImage.load_argb_64_pixels.
	# Обратите внимание, что само изображение должно иметь 16 бит на образец, потому что aspose.imaging.RasterImage.load_argb_64_pixels не работает с 8 битами на образец.
	# Чтобы работать с 8 битами на образец, пожалуйста, используйте проверенный метод aspose.imaging.RasterImage.load_argb_64_pixels.
...
with Image.load("alpha.png") as image:
	rasterImage = as_of(image, RasterImage)
	# Загрузить пиксели для всего изображения. Любую прямоугольную часть изображения можно указать в качестве параметра метода aspose.imaging.RasterImage.load_pixels.
...
# Убрать искажённое сканирование с параметрами по умолчанию
