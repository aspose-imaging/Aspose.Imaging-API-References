---
title: "EmfXHeight"
second_title: "مرجع API لـ Aspose.Imaging للـ Java"
description: "تحدد تعداد XHeight القيم لأحد الخصائص في نظام PANOSE لتصنيف الخطوط."
type: docs
weight: 46
url: /ar/java/com.aspose.imaging.fileformats.emf.emf.consts/emfxheight/
---
**Inheritance:**
java.lang.Object, com.aspose.ms.System.ValueType, com.aspose.ms.System.Enum
```
public final class EmfXHeight extends System.Enum
```

تحدد تعداد XHeight القيم لأحد الخصائص في نظام PANOSE لتصنيف الخطوط.
## الحقول

| حقل | الوصف |
| --- | --- |
| [PAN_ANY](#PAN-ANY) | الارتفاع أي. |
| [PAN_NO_FIT](#PAN-NO-FIT) | عدم الملاءمة. |
| [PAN_XHEIGHT_CONSTANT_SMALL](#PAN-XHEIGHT-CONSTANT-SMALL) | ثابت/صغير |
| [PAN_XHEIGHT_CONSTANT_STD](#PAN-XHEIGHT-CONSTANT-STD) | ثابت/قياسي |
| [PAN_XHEIGHT_CONSTANT_LARGE](#PAN-XHEIGHT-CONSTANT-LARGE) | ثابت/كبير |
| [PAN_XHEIGHT_DUCKING_SMALL](#PAN-XHEIGHT-DUCKING-SMALL) | تخفيض/صغير |
| [PAN_XHEIGHT_DUCKING_STD](#PAN-XHEIGHT-DUCKING-STD) | تخفيض/قياسي |
| [PAN_XHEIGHT_DUCKING_LARGE](#PAN-XHEIGHT-DUCKING-LARGE) | تخفيض/كبير |
### PAN_ANY {#PAN-ANY}
```
public static final byte PAN_ANY
```


الارتفاع أي.

### PAN_NO_FIT {#PAN-NO-FIT}
```
public static final byte PAN_NO_FIT
```


عدم الملاءمة.

### PAN_XHEIGHT_CONSTANT_SMALL {#PAN-XHEIGHT-CONSTANT-SMALL}
```
public static final byte PAN_XHEIGHT_CONSTANT_SMALL
```


ثابت/صغير

### PAN_XHEIGHT_CONSTANT_STD {#PAN-XHEIGHT-CONSTANT-STD}
```
public static final byte PAN_XHEIGHT_CONSTANT_STD
```


ثابت/قياسي

### PAN_XHEIGHT_CONSTANT_LARGE {#PAN-XHEIGHT-CONSTANT-LARGE}
```
public static final byte PAN_XHEIGHT_CONSTANT_LARGE
```


ثابت/كبير

### PAN_XHEIGHT_DUCKING_SMALL {#PAN-XHEIGHT-DUCKING-SMALL}
```
public static final byte PAN_XHEIGHT_DUCKING_SMALL
```


تخفيض/صغير

### PAN_XHEIGHT_DUCKING_STD {#PAN-XHEIGHT-DUCKING-STD}
```
public static final byte PAN_XHEIGHT_DUCKING_STD
```


تخفيض/قياسي

### PAN_XHEIGHT_DUCKING_LARGE {#PAN-XHEIGHT-DUCKING-LARGE}
```
public static final byte PAN_XHEIGHT_DUCKING_LARGE
```


تخفيض/كبير

